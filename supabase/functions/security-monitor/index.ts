...
serve(async (req) => {
  ...
  try {
    ...
    if (error) {
      ...
      return new Response('Error storing scan results', { status: 500, headers: corsHeaders })
    }
...
async function performSecurityScan(supabaseClient: any) {
  ...
  const issues = Object.values(checks).reduce((acc, check) => {
  ...
  const complianceScore = Math.max(100 - (issues.critical * 25 + issues.high * 10 + issues.medium * 5 + issues.low * 1), 0)
