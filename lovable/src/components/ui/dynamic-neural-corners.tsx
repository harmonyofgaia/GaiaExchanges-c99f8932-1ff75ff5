  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-10">
      {effects.map((effect) => (
        <div
          ...
          className={`absolute w-40 h-40 ${
            effect.corner === 'top-left' ? 'top-0 left-0' :
            effect.corner === 'top-right' ? 'top-0 right-0' :
            effect.corner === 'bottom-left' ? 'bottom-0 left-0' :
            'bottom-0 right-0'
          }`}
          style={{
            ...
            }, ${effect.color}${Math.floor(effect.intensity * 255).toString(16).padStart(2, '0')} 0%, transparent 70%)`,
            filter: 'blur(2px)',
      ...
      <svg className="absolute inset-0 w-full h-full">
        <defs>
          <pattern id="neural-circuit" x="0" y="0" width="50" height="50" patternUnits="userSpaceOnUse">
        ...
        <rect x="calc(100% - 100px)" y="0" width="100" height="100" fill="url(#neural-circuit)" opacity="0.3" />
        
        {/* Bottom corners */}
        <rect x="0" y="calc(100% - 100px)" width="100" height="100" fill="url(#neural-circuit)" opacity="0.3" />
        <rect x="calc(100% - 100px)" y="calc(100% - 100px)" width="100" height="100" fill="url(#neural-circuit)" opacity="0.3" />
      </svg>
    </div>
  )
}
