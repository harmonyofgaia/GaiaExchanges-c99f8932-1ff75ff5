...
export function DesignLibrary({ isLocked }: { isLocked: boolean }) {
  ...
  const exportCurrentDesign = () => {
    const designData = {
      ...
      customAssets,
      currentStyles: {
        background: getComputedStyle(document.documentElement).getPropertyValue('--custom-background'),
                ...
                >
                  <div className="relative">
                    ...
                    >
                      <Star 
                        className={`h-4 w-4 ${
                          favorites.includes(asset.id) ? 'fill-yellow-400 text-yellow-400' : 'text-white'
                        }`} 
                      />
                    </Button>
                  </div>
