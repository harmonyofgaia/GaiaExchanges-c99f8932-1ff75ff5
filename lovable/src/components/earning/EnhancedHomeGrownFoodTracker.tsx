...
export function EnhancedHomeGrownFoodTracker() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const stageBonus = basePoints * (stageMultipliers[growthStage as keyof typeof stageMultipliers] || 1)
    const methodBonus = stageBonus * (methodMultipliers[growingMethod as keyof typeof methodMultipliers] || 1)
    ...
    try {
      ...
      toast.success(`🥕 Home grown food recorded! +${totalPoints} points earned for sustainable growing!`)
      setCropType('')
      setQuantity('')
      setGrowthStage('')
      setGrowingMethod('')
  ...
  const getStageColor = (stage: string) => {
    switch (stage) {
      case 'seedling': return 'bg-green-600'
      case 'growing': return 'bg-blue-600'
      case 'flowering': return 'bg-yellow-600'
      case 'fruiting': return 'bg-orange-600'
      case 'harvest_ready': return 'bg-red-600'
      case 'harvested': return 'bg-purple-600'
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {currentCrops.map((crop) => (
              <div key={crop.id} className="p-4 bg-green-900/20 rounded-lg border border-green-500/20">
        ...
        <div className="space-y-3">
          ...
          {recentHarvests.map((harvest) => (
            <div key={harvest.id} className="flex items-center justify-between p-3 bg-orange-900/20 rounded-lg border border-orange-500/20">
              <div className="flex items-center gap-3">
                <Scale className="h-5 w-5 text-orange-400" />
                <div>
                  <div className="font-medium text-orange-300">{harvest.crop}</div>
                  <div className="text-sm text-muted-foreground">
                    {harvest.weight}kg • {harvest.method.replace('_', ' ')} • {harvest.photos} photos
                  </div>
                </div>
              </div>
              <div className="text-right">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-gradient-to-r from-orange-900/20 to-green-900/20 rounded-lg border border-orange-500/20">
          <h4 className="font-semibold text-orange-400 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Harvest Weight (kg) - Optional</label>
              <Input
                type="number"
                step="0.1"
                value={harvestWeight}
            ...
            <div>
              ...
              <Button type="button" variant="outline" className="w-full border-green-500 text-green-400">
        ...
        <div className="p-4 bg-gradient-to-r from-orange-500/10 to-green-500/10 rounded-lg border border-orange-500/20">
          <p className="text-sm text-orange-300">
            💡 <strong>Garden Master Bonus:</strong> Different growing methods and stages earn different multipliers! Photo verification and detailed tracking earn the highest rewards!
          </p>
        </div>
      </CardContent>
    </Card>
