...
export function GaiaBikeEarning() {
  ...
  const startBikeSession = () => {
    ...
    toast.success('Bike Session Started!', {
      description: 'Your GAiA bike earning session is now active. Earn 2 tokens per km!',
      duration: 4000
    })
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-blue-400">Current Session</h3>
              ...
              {currentSession ? (
                <div className="p-4 bg-blue-900/30 rounded-lg border border-blue-500/20">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="text-blue-400 font-bold">{currentSession.distance.toFixed(2)} km</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Tokens Earning:</span>
                      <span className="text-green-400 font-bold">{currentSession.tokensEarned} GAiA</span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className="p-4 bg-gray-900/30 rounded-lg border border-gray-500/20 text-center">
                  ...
                  <p className="text-muted-foreground mb-4">Ready to start earning with your GAiA bike?</p>
                  <Button 
            ...
            {/* Stats & History */}
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-blue-400">Your Stats</h3>
              ...
              <div className="space-y-2">
                ...
                {sessionHistory.slice(0, 3).map((session) => (
                  <div key={session.id} className="p-2 bg-muted/20 rounded border border-border/50">
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">{session.route}</span>
                      <span className="text-green-400">+{session.tokensEarned} GAiA</span>
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {session.distance.toFixed(1)}km • {session.startTime.toLocaleDateString()}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="mt-6 p-4 bg-cyan-900/20 rounded-lg border border-cyan-500/30">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-medium text-cyan-400">Earning Rate: 2 GAiA per KM</span>
            </div>
            <p className="text-sm text-cyan-300/80">
              💡 <strong>Bonus multipliers:</strong> Group rides (1.5x), eco-routes (1.3x), weekly challenges (2x)
            </p>
          </div>
        </CardContent>
      </Card>
