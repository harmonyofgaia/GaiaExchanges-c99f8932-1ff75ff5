...
export function EnergyConsumptionTracker() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const timeframeBonus = basePoints * (timeframeMult[timeframe as keyof typeof timeframeMult] || 1)
    const sourceBonus = energySource ? timeframeBonus * (sourceMult[energySource as keyof typeof sourceMult] || 1) : timeframeBonus
    ...
    toast.success(`⚡ Energy reduction recorded! +${totalPoints} points earned for ${reductionPercent.toFixed(1)}% efficiency gain!`)
    setEnergyType('')
    setCurrentUsage('')
    setPreviousUsage('')
    setTimeframe('')
    ...
    <Card className="border-yellow-500/30 bg-gradient-to-br from-yellow-900/20 via-orange-900/20 to-red-900/20">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-yellow-400">
          ...
          ⚡ Energy Consumption & Efficiency Tracker
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div className="text-center p-4 bg-yellow-900/30 rounded-lg border border-yellow-500/20">
              <div className="text-xl font-bold text-yellow-400">{liveData.currentPower.toFixed(1)}kW</div>
            ...
            <div className="text-center p-4 bg-orange-900/30 rounded-lg border border-orange-500/20">
              <div className="text-xl font-bold text-orange-400">{liveData.dailyUsage.toFixed(1)}kWh</div>
            ...
            <div className="text-center p-4 bg-green-900/30 rounded-lg border border-green-500/20">
              <div className="text-xl font-bold text-green-400">{liveData.efficiency.toFixed(0)}%</div>
            ...
            <div className="text-center p-4 bg-red-900/30 rounded-lg border border-red-500/20">
              <div className="text-xl font-bold text-red-400">{liveData.co2Emissions.toFixed(1)}kg</div>
            ...
            <div className="text-center p-4 bg-purple-900/30 rounded-lg border border-purple-500/20">
              <div className="text-xl font-bold text-purple-400">${liveData.costSavings.toFixed(2)}</div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-4">
            ...
            <div className="text-center p-4 bg-blue-900/30 rounded-lg border border-blue-500/20">
              <div className="text-xl font-bold text-blue-400">{monthlyStats.renewablePercent}%</div>
        ...
        <div className="space-y-3">
          ...
          {recentEfforts.map((effort) => (
            <div key={effort.id} className="flex items-center justify-between p-3 bg-yellow-900/20 rounded-lg border border-yellow-500/20">
              <div className="flex items-center gap-3">
                <TrendingDown className="h-5 w-5 text-green-400" />
                <div>
                  <div className="font-medium text-yellow-300">{effort.action}</div>
                  <div className="text-sm text-muted-foreground">
                    -{effort.reduction}kWh {effort.timeframe} • ${effort.savings} saved • {effort.date}
                  </div>
                </div>
              </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-gradient-to-r from-yellow-900/20 to-orange-900/20 rounded-lg border border-yellow-500/20">
          <h4 className="font-semibold text-yellow-400 flex items-center gap-2">
            ...
            <div className="p-3 bg-green-900/20 rounded-lg border border-green-500/20">
              <div className="text-center">
                <div className="text-2xl font-bold text-green-400">
                  {(((parseFloat(previousUsage) - parseFloat(currentUsage)) / parseFloat(previousUsage)) * 100).toFixed(1)}%
                </div>
        ...
        <div className="p-4 bg-gradient-to-r from-yellow-500/10 to-orange-500/10 rounded-lg border border-yellow-500/20">
          <p className="text-sm text-yellow-300">
            💡 <strong>Efficiency Master:</strong> Large percentage reductions earn 1.5x bonus! Renewable energy sources provide multiplied rewards for sustainable living!
          </p>
        </div>
      </CardContent>
    </Card>
