...
export function PrivateChatrooms({ selectedLocation, playerData, isAdmin }: ChatroomProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      user: 'System',
      content: '🛡️ Welcome to the quantum-encrypted chatroom. All communications are secured by parabolic universe technology.',
      timestamp: new Date(),
      encrypted: true
    }
  ])
  ...
  const [availableChatrooms] = useState<Chatroom[]>([
    { id: '1', name: 'Global Secure Chat', participants: 156, private: false, location: 'Global', admin: false },
    { id: '2', name: 'VIP Members Only', participants: 42, private: true, location: 'Global', admin: false },
    { id: '3', name: 'Local Community', participants: 23, private: false, location: selectedLocation?.city || 'Unknown', admin: false },
    { id: '4', name: '👑 Admin Command Center', participants: 1, private: true, location: 'Secure', admin: true }
  ])
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setParabolicConnection(prev => Math.max(95, Math.min(100, prev + (Math.random() - 0.5) * 2)))
              ...
              <div className="space-y-4">
                ...
                <div className="p-3 bg-green-900/20 rounded-lg border border-green-500/30">
                  <div className="text-xs text-green-300 space-y-1">
                    ...
                    {isAdmin && <div className="text-red-300">👑 Admin: Full monitoring capabilities active</div>}
                  </div>
                </div>
              </div>
            ) : (
