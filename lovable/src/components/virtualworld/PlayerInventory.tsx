...
export function PlayerInventory() {
  ...
  const equipment = [
    { name: '🗡️ Eco-Warrior Sword', type: 'weapon', power: 250, rarity: 'legendary', equipped: true },
    { name: '🛡️ Forest Guardian Shield', type: 'armor', defense: 180, rarity: 'epic', equipped: true },
    { name: '👑 Crown of Nature', type: 'helmet', power: 120, rarity: 'rare', equipped: false },
    { name: '🌊 Ocean Trident', type: 'weapon', power: 300, rarity: 'legendary', equipped: false }
  ]
  ...
  const achievements = [
    { name: '🌱 Tree Planter', description: 'Burned 1000 tokens for tree planting', unlocked: true },
    { name: '🐋 Ocean Protector', description: 'Saved 500 marine animals', unlocked: true },
    { name: '🔥 Token Burner Elite', description: 'Burned 5000+ tokens', unlocked: false },
    { name: '🌍 Eco Champion', description: 'Completed all environmental quests', unlocked: false }
  ]

  const resources = [
    { name: 'Environmental Tokens', amount: 2847, icon: Flame, color: 'text-orange-400' },
    { name: 'Nature Crystals', amount: 156, icon: Gem, color: 'text-purple-400' },
    { name: 'Eco Points', amount: 9240, icon: Star, color: 'text-yellow-400' },
    { name: 'Conservation Energy', amount: 750, icon: Zap, color: 'text-blue-400' }
  ]
              ...
              <Card key={index} className={`border transition-all ${
                ...
                <CardContent className="pt-4">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {item.equipped && (
                      <Badge className="bg-green-600 text-white">EQUIPPED</Badge>
                    )}
                  </div>
        ...
        <TabsContent value="landscapes" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {landscapes.map((landscape, index) => (
              <Card key={index} className={`border transition-all ${
                landscape.owned ? 'border-green-500/50 bg-green-900/20' : 'border-red-500/30 opacity-70'
              }`}>
                <CardContent className="pt-4">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-orange-400 font-bold">{landscape.tokensInvested}</span>
                        </div>
                      </div>
        ...
        <TabsContent value="achievements" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {achievements.map((achievement, index) => (
              <Card key={index} className={`border transition-all ${
                achievement.unlocked ? 'border-yellow-500/50 bg-yellow-900/20' : 'border-muted/50 opacity-70'
              }`}>
                <CardContent className="pt-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
