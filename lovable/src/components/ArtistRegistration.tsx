...
export function ArtistRegistration() {
  ...
  const [fundAllocations, setFundAllocations] = useState<FundAllocation[]>([
    { project: 'Environmental Conservation', percentage: 25, description: 'Tree planting and wildlife protection', icon: <TreePine className="h-4 w-4" /> },
    { project: 'Community Support', percentage: 25, description: 'Local community development projects', icon: <Heart className="h-4 w-4" /> },
    { project: 'Platform Development', percentage: 25, description: 'Improving GAiA platform features', icon: <Zap className="h-4 w-4" /> },
    { project: 'Artist Fund', percentage: 25, description: 'Supporting other emerging artists', icon: <Palette className="h-4 w-4" /> }
  ])
  ...
  const handlePercentageChange = (index: number, newPercentage: number) => {
    ...
    if (total !== 100) {
      ...
      otherIndices.forEach(i => {
        updatedAllocations[i].percentage = Math.max(0, updatedAllocations[i].percentage + adjustment)
      })
    }
  ...
  const handleFundAllocationSave = async () => {
    try {
      ...
      const burnSuccess = await gaiaTokenService.burnTokens(50, `Fund allocation setup for ${artistData.name}`)
          ...
          <Card className="border-2 border-green-500/50 bg-gradient-to-br from-green-900/20 to-emerald-900/20">
            <CardContent className="pt-6">
              <div className="text-center">
                ...
                <h3 className="text-2xl font-bold text-green-400 mb-2">Welcome, {artistData.name}!</h3>
                <p className="text-muted-foreground">You're now registered as a GAiA artist and can start streaming.</p>
