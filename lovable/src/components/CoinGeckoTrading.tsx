    ...
    <Card className="bg-gradient-to-br from-yellow-900/30 to-orange-900/30 border-yellow-500/30">
      <CardHeader>
        ...
        <div className="flex items-center gap-2">
          <Button onClick={openCoinGecko} variant="outline" size="sm" className="border-yellow-500/30 text-yellow-400">
            ...
            <div key={index} className="flex items-center justify-between p-3 bg-black/30 rounded-lg border border-gray-500/30 hover:border-yellow-500/50 transition-colors">
              ...
              <div className="text-right">
                <div className="font-bold text-white">
                  {coin.symbol === 'GAiA' ? formatGaiaPrice(coin.price) : `$${coin.price.toLocaleString()}`}
                </div>
                <Badge className={`${coin.change24h >= 0 ? 'bg-green-600' : 'bg-red-600'} text-white`}>
