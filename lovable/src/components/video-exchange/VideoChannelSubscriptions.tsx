...
export function VideoChannelSubscriptions() {
  const [subscriptions, setSubscriptions] = useState<ChannelSubscription[]>([
    {
      ...
      avatar: 'https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?w=64&h=64&fit=crop&crop=face',
      subscribers: 45600,
      contentType: 'music',
      isSubscribed: true,
      isLive: true,
    ...
    },
    {
      ...
      avatar: 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=64&h=64&fit=crop&crop=face',
      subscribers: 23400,
      contentType: 'music',
      isSubscribed: true,
      isLive: false,
  ...
  const handleSubscribe = (channelId: string) => {
    setSubscriptions(prev => prev.map(sub => 
      sub.id === channelId 
        ? { ...sub, isSubscribed: !sub.isSubscribed, subscribers: sub.isSubscribed ? sub.subscribers - 1 : sub.subscribers + 1 }
        : sub
    ))
    ...
    if (channel) {
      toast.success(
        channel.isSubscribed ? `Unsubscribed from ${channel.channelName}` : `Subscribed to ${channel.channelName}! 🔔`,
        {
  ...
  const filteredChannels = subscriptions.filter(channel =>
    ...
    <div className="space-y-6">
      <Card className="border-blue-500/30 bg-gradient-to-r from-blue-900/20 to-purple-900/20">
        <CardHeader>
          ...
          <p className="text-muted-foreground">Follow your favorite creators and discover new talent</p>
        </CardHeader>
        <CardContent>
          {/* Search */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="p-4 bg-muted rounded-lg text-center">
              <div className="text-2xl font-bold text-blue-400">{subscriptions.filter(s => s.isSubscribed).length}</div>
              <div className="text-sm text-muted-foreground">Subscribed Channels</div>
            </div>
            <div className="p-4 bg-muted rounded-lg text-center">
              <div className="text-2xl font-bold text-green-400">{subscriptions.filter(s => s.isLive && s.isSubscribed).length}</div>
              <div className="text-sm text-muted-foreground">Live Now</div>
            </div>
            <div className="p-4 bg-muted rounded-lg text-center">
              <div className="text-2xl font-bold text-purple-400">{subscriptions.filter(s => s.contentType === 'music').length}</div>
          ...
          <div className="space-y-4">
            {filteredChannels.map((channel) => (
              <Card key={channel.id} className="border-gray-700 hover:border-blue-500/50 transition-colors">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      {channel.isLive && (
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="text-sm text-red-400 font-medium">Currently streaming live</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                      <Button
                        ...
                        className={channel.isSubscribed ? "bg-gray-600" : "bg-red-600 hover:bg-red-700"}
                      >
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 bg-muted rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground">Discover emerging artists creating eco-conscious music</p>
                </div>
                <div className="p-4 bg-muted rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm text-muted-foreground">Learn from sustainability experts and activists</p>
                </div>
                <div className="p-4 bg-muted rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm text-muted-foreground">Connect with local environmental communities</p>
                </div>
              </div>
            </CardContent>
          </Card>
