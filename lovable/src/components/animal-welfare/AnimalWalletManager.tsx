...
export function AnimalWalletManager({ animals }: AnimalWalletManagerProps) {
  ...
  const totalFunding = animals.reduce((sum, animal) => sum + animal.currentFunding, 0);
  const totalGoals = animals.reduce((sum, animal) => sum + animal.fundingGoal, 0);
      ...
      <Card className="border-yellow-500/30">
        <CardHeader>
          ...
          <p className="text-muted-foreground">
            Every animal in captivity has its own dedicated wallet. All contributions go directly to
            their care and release fund.
          </p>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center p-4 bg-blue-900/30 rounded border border-blue-500/20">
              <div className="text-2xl font-bold text-blue-400">{totalGoals.toLocaleString()}</div>
              <div className="text-sm text-muted-foreground">Combined Goals</div>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-black/40 p-4 rounded border border-green-500/20">
                  <div className="flex items-center justify-between mb-2">
                    <div className="font-semibold text-green-400">🔐 Dedicated Wallet</div>
                    <Button
                      size="sm"
                      variant="outline"
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" className="flex-1 border-blue-500/50">
                    <Eye className="h-4 w-4 mr-2" />
                    View Transactions
                  </Button>
                  <Button variant="outline" size="sm" className="flex-1 border-purple-500/50">
                ...
                <div className="bg-blue-900/20 p-3 rounded border border-blue-500/20">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm font-semibold text-blue-400">100% Transparent</span>
                  </div>
                  <p className="text-xs text-blue-300">
                    Every transaction is recorded on the blockchain. Funds are used exclusively for
                    food, medical care, habitat improvement, and release preparation.
                  </p>
                </div>
              </div>
            </CardContent>
