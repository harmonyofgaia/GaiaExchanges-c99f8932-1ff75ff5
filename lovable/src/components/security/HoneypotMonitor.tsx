          ...
          <CardContent>
            <div className="space-y-3 max-h-64 overflow-y-auto">
              {logs.slice(-10).reverse().map((log, index) => (
                <div key={index} className="p-3 bg-black/30 rounded-lg border border-yellow-500/20">
              ...
              {attempts.slice(-10).reverse().map((attempt, index) => (
                <div key={index} className="p-3 bg-black/30 rounded-lg border border-red-500/20">
                  ...
                  <div className="text-xs text-gray-300 space-y-1">
                    ...
                    <div>Username: <span className="text-red-400">{attempt.username || 'N/A'}</span></div>
                    <div>Password: <span className="text-red-400">{attempt.password ? '•'.repeat(attempt.password.length) : 'N/A'}</span></div>
                  </div>
                </div>
              ))}
            </div>
