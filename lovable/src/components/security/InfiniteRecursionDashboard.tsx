...
import { Infinity, RotateCcw, Zap, Target, AlertCircle, Atom } from 'lucide-react'
    ...
    <Card className="border-purple-500/20 bg-gradient-to-br from-purple-950/20 to-indigo-950/20">
      ...
      <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
            {patterns.map((pattern) => (
              <div key={pattern.id} className="flex justify-between">
                ...
                <span className="text-purple-400">{pattern.depth.toLocaleString()} / {pattern.stability}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="text-xs text-muted-foreground">
          <p className="italic">
            "In infinite recursion, every paradox becomes truth, every impossibility becomes inevitable. 
            Logic itself bends to our will."
          </p>
        </div>
      </CardContent>
    </Card>
