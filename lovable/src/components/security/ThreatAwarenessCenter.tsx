...
export function ThreatAwarenessCenter() {
  const [threatTypes] = useState<ThreatType[]>([
    ...
    {
      name: 'Data Thieves',
      description: 'Attackers attempting to steal personal information, seed phrases, and financial data',
      severity: 'high',
      blockedToday: 34,
      detectionRate: 96.8
    },
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      ...
      default: return 'text-gray-500 border-gray-500/50 bg-gray-900/20'
    }
  }
        ...
        {threatTypes.map((threat, index) => (
          <Card key={index} className={`border ${getSeverityColor(threat.severity)}`}>
            <CardHeader className="pb-3">
              <CardTitle className={`text-lg ${threat.severity === 'critical' ? 'text-red-400' : threat.severity === 'high' ? 'text-orange-400' : threat.severity === 'medium' ? 'text-yellow-400' : 'text-green-400'}`}>
                {threat.name}
              </CardTitle>
            </CardHeader>
        ...
        <CardContent className="pt-6">
          <div className="text-center space-y-4">
            ...
            <div className="max-w-4xl mx-auto space-y-3 text-green-200">
              <p className="text-lg">
                We are committed to creating a transparent, undestructible way of life for our community.
                Our advanced security systems work tirelessly to protect every member from cyber threats.
              </p>
              ...
              <div className="mt-6 p-4 bg-gradient-to-r from-yellow-900/30 to-red-900/30 rounded-lg border border-yellow-500/30">
                ...
                <p className="text-sm text-center">
                  Every threat against our community is met with immediate and decisive action.
                  We don't just defend - we actively fight back to protect what we've built together.
                </p>
              </div>
