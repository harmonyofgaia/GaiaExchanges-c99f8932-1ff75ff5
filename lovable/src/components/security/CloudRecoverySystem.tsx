...
export function CloudRecoverySystem() {
  ...
  const initiateDeviceRecovery = (deviceId: string) => {
    ...
    setRecoveryDevices(prev => prev.map(device => 
      device.id === deviceId 
        ? { ...device, recoveryReady: true, dataSync: 100, lastAccess: new Date() }
        : device
    ))
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-4 rounded-lg bg-blue-900/30 border border-blue-500/20">
              ...
              <div className="text-2xl font-bold text-blue-400">{recoveryStats.totalDataMirrored}GB</div>
        ...
        <CardContent>
          <div className="space-y-4">
            {recoveryDevices.map((device) => (
              <div key={device.id} className="flex items-center justify-between p-4 rounded-lg bg-muted/30 border">
