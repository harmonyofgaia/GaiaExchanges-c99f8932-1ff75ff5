...
export function GameStatusIndicator() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setGameStatus((prev) => ({
        ...
        serverLoad: Math.max(
          20,
          Math.min(95, prev.serverLoad + (Math.random() - 0.5) * 10),
        ),
        latency: Math.max(
          20,
  ...
  const getStatusColor = () => {
    ...
    if (gameStatus.serverLoad > 80 || gameStatus.latency > 150)
    ...
    <Card className="bg-gradient-to-br from-gray-900/50 to-green-900/30 border border-green-500/30">
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <Badge className={`${getStatusColor()} bg-black/30`}>
            {getStatusText()}
          </Badge>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div className="text-center">
            ...
            <div className="font-bold text-blue-400">
              {gameStatus.playerCount.toLocaleString()}
            </div>
