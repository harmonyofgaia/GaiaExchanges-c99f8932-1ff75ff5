    ...
    evolutionInterval.current = setInterval(() => {
      ...
      if (Math.random() < 0.2) {
        setDefenseAnimals((prev) => {
          ...
          const statuses: DefenseAnimal["status"][] = [
            "active",
            "hunting",
            "defending",
            "patrolling",
        ...
        <Card className="bg-gradient-to-br from-purple-900/30 to-indigo-900/30 border-purple-500/30">
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold text-purple-400">
              {defenseAnimals.reduce((sum, a) => sum + a.threatsRepelled, 0)}
            </div>
        ...
        <Card className="bg-gradient-to-br from-yellow-900/30 to-amber-900/30 border-yellow-500/30">
          <CardContent className="p-4 text-center">
            ...
            <div className="text-2xl font-bold text-yellow-400">
              {(
                defenseAnimals.reduce((sum, a) => sum + a.trainingProgress, 0) /
              ...
              <Card
                ...
                className={`cursor-pointer transition-all duration-300 hover:scale-105 border-2 ${
                  animal.status === "active"
                    ...
                    : animal.status === "defending"
                      ? "border-red-500/50 bg-red-900/20"
                      : animal.status === "hunting"
                        ? "border-orange-500/50 bg-orange-900/20"
                        : animal.status === "training"
                          ? "border-blue-500/50 bg-blue-900/20"
                          : animal.status === "patrolling"
                  ...
                  <div className="text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-green-400 font-bold">
                        {animal.effectiveness.toFixed(1)}%
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Threats:</span>
        ...
        <TabsContent value="patterns" className="space-y-4">
          <div className="grid gap-4">
            {attackPatterns.map((pattern) => (
              <Card
                key={pattern.id}
                className="bg-gradient-to-r from-gray-900/50 to-gray-800/50 border-gray-700/50"
              >
                <CardHeader>
                  <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {pattern.animalTypes.map((type) => {
                      ...
                      return animal ? (
                        <span
                          key={type}
                          className="px-2 py-1 bg-purple-700/30 rounded text-xs text-purple-300 flex items-center gap-1"
                        >
              ...
              <CardContent className="space-y-4">
                <div className="text-sm space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-bold ${metrics.globalScanningActive ? "text-green-400" : "text-red-400"}`}
                    >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center p-3 bg-red-900/30 rounded-lg">
                    <div className="text-xl font-bold text-red-400">
                      {selectedAnimal.powerLevel.toLocaleString()}
                    </div>
                    <div className="text-xs text-red-300">Power Level</div>
                  </div>
                  <div className="text-center p-3 bg-green-900/30 rounded-lg">
                    <div className="text-xl font-bold text-green-400">
                      {selectedAnimal.effectiveness.toFixed(1)}%
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedAnimal.specialAbilities.map((ability, index) => (
                      <Badge
                        key={index}
                        variant="secondary"
                        className="bg-purple-700/30 text-purple-300"
                      >
                        {ability}
                      </Badge>
                    ))}
                  ...
                  <Badge
                    className={`ml-2 ${
                      selectedAnimal.status === "active"
                        ? "bg-green-600"
                        : selectedAnimal.status === "defending"
                          ? "bg-red-600"
                          : selectedAnimal.status === "hunting"
                            ? "bg-orange-600"
                            : selectedAnimal.status === "training"
                              ? "bg-blue-600"
                              : selectedAnimal.status === "patrolling"
                                ? "bg-purple-600"
                                : "bg-gray-600"
                    }`}
                  >
