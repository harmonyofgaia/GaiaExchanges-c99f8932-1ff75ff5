...
export function ServerResilienceEngine() {
  ...
  useEffect(() => {
    const securityMonitoring = setInterval(() => {
      ...
      const avgStrength = securityLayers.reduce((sum, layer) => sum + layer.strength, 0) / securityLayers.length
        ...
        <CardContent>
          <div className="space-y-4">
            {securityLayers.map((layer) => (
              <div key={layer.id} className="flex items-center justify-between p-4 rounded-lg bg-muted/30">
        ...
        <CardContent>
          <div className="space-y-4">
            {serverNodes.map((node) => (
              <div key={node.id} className="flex items-center justify-between p-4 rounded-lg bg-muted/30">
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Badge className={
                    ...
                    node.status === 'offline' ? 'bg-red-600' : 'bg-yellow-600'
                  }>
                    {node.status}
                  </Badge>
                </div>
