...
export function UniversalSystemController() {
  ...
  useEffect(() => {
    ...
    const simulateSystemDiscovery = () => {
      if (Math.random() > 0.7) {
        const systemTypes = ['computer', 'server', 'mobile', 'router', 'database', 'cloud'] as const
        const systemNames = [
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'transcended': return 'bg-purple-600'
      case 'dominated': return 'bg-red-600'
      case 'controlled': return 'bg-green-600'
            ...
            <div key={system.id} className="p-3 bg-black/40 rounded-lg border border-blue-500/30">
              ...
              <div className="text-xs space-y-1">
                <div className="text-blue-400">
                  Power Level: {system.powerLevel}% • Control Methods: {system.controlMethods.length}
                </div>
