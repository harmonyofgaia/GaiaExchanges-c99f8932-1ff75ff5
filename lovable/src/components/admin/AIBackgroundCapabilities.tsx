...
export function AIBackgroundCapabilities() {
  ...
  useEffect(() => {
    ...
    const trainingInterval = setInterval(() => {
      setCapabilities(prev => prev.map(capability => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'evolving': return 'bg-purple-600'
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center p-4 bg-purple-900/20 rounded-lg border border-purple-500/20">
            <div className="text-3xl font-bold text-purple-400">{overallIntelligence.toFixed(1)}%</div>
          ...
          <div className="text-center p-4 bg-blue-900/20 rounded-lg border border-blue-500/20">
            <div className="text-3xl font-bold text-blue-400">{totalImprovements.toLocaleString()}</div>
          ...
          <div className="text-center p-4 bg-green-900/20 rounded-lg border border-green-500/20">
            <div className="text-3xl font-bold text-green-400">{trainingCycles.toLocaleString()}</div>
          ...
          {capabilities.map((capability, index) => (
            <div key={index} className="p-4 rounded-lg bg-black/20 border border-purple-500/20">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-purple-400">{capability.level.toFixed(1)}%</span>
        ...
        <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <div className="text-purple-400">Learning admin preferences and reducing repetitive issues</div>
            </div>
            <div>
              <div className="text-blue-300 mb-1">Error Prevention:</div>
              <div className="text-blue-400">Developing better consistency in system operations</div>
            </div>
            <div>
              <div className="text-green-300 mb-1">Problem Solving:</div>
              <div className="text-green-400">Enhancing automatic issue detection and resolution</div>
            </div>
            <div>
              <div className="text-yellow-300 mb-1">System Integration:</div>
              <div className="text-yellow-400">Optimizing multi-component interactions and stability</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="text-blue-400 font-bold">Target: 95% (Current: {(100 - (totalImprovements / 100)).toFixed(1)}%)</span>
            </div>
            <div className="flex justify-between">
              <span className="text-purple-300">Improve Response Accuracy:</span>
              <span className="text-purple-400 font-bold">Target: 98% (Current: {overallIntelligence.toFixed(1)}%)</span>
            </div>
            <div className="flex justify-between">
              <span className="text-green-300">Eliminate Recurring Errors:</span>
              <span className="text-green-400 font-bold">Target: 100% (Current: {capabilities[1]?.level.toFixed(1)}%)</span>
            </div>
          </div>
        </div>
      </CardContent>
