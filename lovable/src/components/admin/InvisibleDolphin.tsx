...
export function InvisibleDolphin() {
      ...
      const interval = setInterval(() => {
        ...
        if (Math.random() < 0.4) {
          const newNode: DataNode = {
            ...
            type: ['encrypted', 'deleted', 'hidden', 'classified'][Math.floor(Math.random() * 4)] as any,
          ...
          if (newNode.extracted) {
            setMetrics(prev => ({
              ...
              encryptedData: newNode.type === 'encrypted' ? prev.encryptedData + 1 : prev.encryptedData,
              deletedRecovered: newNode.type === 'deleted' ? prev.deletedRecovered + 1 : prev.deletedRecovered,
              hiddenRevealed: newNode.type === 'hidden' ? prev.hiddenRevealed + 1 : prev.hiddenRevealed,
              classifiedAccessed: newNode.type === 'classified' ? prev.classifiedAccessed + 1 : prev.classifiedAccessed
            }))
          }
        }
      }, 2000)
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'classified': return <Shield className="h-4 w-4" />
        ...
        <CardContent className="space-y-6">
          ...
          <div className="text-center space-y-4">
            ...
            <div className="space-y-2">
              ...
              {isActive && (
                <div className="space-y-2">
                  <div className="text-sm text-cyan-400">Swimming through encrypted networks...</div>
                  <Progress value={scanProgress} className="w-full" />
                  <div className="text-xs text-muted-foreground">{scanProgress.toFixed(1)}% Complete</div>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Card className="bg-orange-900/20 border-orange-500/30">
              <CardContent className="pt-4 text-center">
                ...
                <div className="text-2xl font-bold text-orange-400">{metrics.classifiedAccessed}</div>
            ...
            <CardContent className="pt-4">
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {dataNodes.length === 0 ? (
                  ...
                  dataNodes.map(node => (
                    <div key={node.id} className="flex items-center justify-between p-3 bg-muted/10 rounded border border-border/20">
                      ...
                      <div className="flex items-center gap-2">
                        <Badge className={`${getTypeColor(node.type)} border-current`} variant="outline">
