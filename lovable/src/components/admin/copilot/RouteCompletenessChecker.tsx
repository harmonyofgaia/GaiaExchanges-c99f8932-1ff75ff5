...
import { 
  Globe, Search, CheckCircle, XCircle, AlertTriangle,
  Code, FileText, Link, Eye, Plus, RefreshCw, Download,
  Route, Component, Settings, ExternalLink, Wrench
} from 'lucide-react'
...
export default ${route.component}
  ...
  const getStatusColor = (status: PageRoute['status']) => {
    switch (status) {
      ...
      case 'incomplete': return 'text-yellow-400 border-yellow-400'
      case 'missing': return 'text-red-400 border-red-400'
      case 'error': return 'text-red-500 border-red-500'
      case 'off-spec': return 'text-orange-400 border-orange-400'
    }
  }
      ...
      <CardHeader>
        ...
        {analysis && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-400">{analysis.coverage.toFixed(0)}%</div>
      ...
      <CardContent>
        ...
        <Tabs defaultValue="routes" className="w-full">
          ...
          <TabsContent value="issues" className="space-y-3">
            {routes.filter(r => r.issues.length > 0).map((route) => (
