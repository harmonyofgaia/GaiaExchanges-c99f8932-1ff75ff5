...
export function GlobalDominationSuite() {
  const [dominationMetrics, setDominationMetrics] = useState<DominationMetric[]>([
    { sector: 'Financial Markets', control_percentage: 87, influence_score: 94, assets_controlled: 2847, revenue_generated: 184750000, status: 'controlling' },
    { sector: 'Social Media Platforms', control_percentage: 92, influence_score: 98, assets_controlled: 15847, revenue_generated: 94750000, status: 'dominated' },
    { sector: 'Government Networks', control_percentage: 76, influence_score: 83, assets_controlled: 547, revenue_generated: 54750000, status: 'controlling' },
    { sector: 'Cryptocurrency Exchanges', control_percentage: 94, influence_score: 97, assets_controlled: 847, revenue_generated: 247500000, status: 'dominated' },
    { sector: 'Tech Giants', control_percentage: 68, influence_score: 71, assets_controlled: 247, revenue_generated: 375000000, status: 'infiltrating' },
    { sector: 'Media Corporations', control_percentage: 89, influence_score: 91, assets_controlled: 1247, revenue_generated: 84750000, status: 'controlling' },
    { sector: 'Educational Institutions', control_percentage: 73, influence_score: 78, assets_controlled: 3847, revenue_generated: 24750000, status: 'controlling' },
    { sector: 'Healthcare Systems', control_percentage: 81, influence_score: 85, assets_controlled: 1847, revenue_generated: 147500000, status: 'controlling' }
  ])
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setDominationMetrics(prev => prev.map(metric => ({
        ...metric,
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'infiltrating': return 'bg-yellow-600'
      case 'controlling': return 'bg-orange-600'
      case 'dominated': return 'bg-green-600'
        ...
        <CardContent className="space-y-6">
            ...
            <CardContent className="pt-4">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div className="text-center">
                  ...
                  <div className="text-2xl font-bold text-red-400">{globalStats.totalDomination.toFixed(1)}%</div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-2xl font-bold text-orange-400">{globalStats.worldInfluence.toFixed(1)}%</div>
                  <div className="text-sm text-muted-foreground">Global Influence</div>
                </div>
                <div className="text-center">
                  <DollarSign className="h-8 w-8 text-green-400 mx-auto mb-2" />
                  <div className="text-2xl font-bold text-green-400">{globalStats.economicControl.toFixed(1)}%</div>
                  <div className="text-sm text-muted-foreground">Economic Control</div>
                </div>
                <div className="text-center">
                  <Eye className="h-8 w-8 text-blue-400 mx-auto mb-2" />
                  <div className="text-2xl font-bold text-blue-400">{globalStats.informationControl.toFixed(1)}%</div>
                  <div className="text-sm text-muted-foreground">Information Control</div>
                </div>
                <div className="text-center">
                  <Users className="h-8 w-8 text-purple-400 mx-auto mb-2" />
                  <div className="text-2xl font-bold text-purple-400">{formatNumber(globalStats.populationReach)}</div>
                  <div className="text-sm text-muted-foreground">Population Reach</div>
                </div>
                <div className="text-center">
                  <Target className="h-8 w-8 text-pink-400 mx-auto mb-2" />
                  <div className="text-2xl font-bold text-pink-400">{globalStats.activeOperations}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {dominationMetrics.map((metric, index) => (
              <Card key={index} className="bg-black/40 border-gray-600/30 hover:border-yellow-500/50 transition-colors">
                <CardContent className="pt-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-yellow-400">{metric.control_percentage.toFixed(1)}%</span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <div className="font-bold text-green-400">{metric.assets_controlled.toLocaleString()}</div>
                      </div>
                      <div>
                        <div className="text-gray-400">Revenue</div>
                        <div className="font-bold text-green-400">{formatCurrency(metric.revenue_generated)}</div>
                      </div>
                    </div>
                  </div>
                </CardContent>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="text-purple-400">{globalStats.economicControl.toFixed(1)}%</span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="text-purple-400">{globalStats.informationControl.toFixed(1)}%</span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="text-purple-400">{((globalStats.populationReach / 8e9) * 100).toFixed(1)}%</span>
                  </div>
