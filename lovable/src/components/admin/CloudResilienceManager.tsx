...
export function CloudResilienceManager() {
  ...
  const forceFullBackup = () => {
    toast.success("💾 Full System Backup Initiated!", {
      description:
        "Complete community data backup across all global nodes - Maximum protection activated",
      duration: 8000,
    });
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="text-center text-sm text-muted-foreground">
              {(cloudMetrics.usedStorage / 1000).toFixed(1)}TB Used of{" "}
              {(cloudMetrics.totalStorageCapacity / 1000).toFixed(0)}TB Total
            </div>
          </div>
