...
export function IAEngine() {
  const [status, setStatus] = useState<'offline' | 'initializing' | 'active' | 'supercharged'>('offline')
  ...
  const getStatusColor = () => {
    switch (status) {
      ...
      case 'supercharged': return 'bg-purple-500'
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">{metrics.intelligence.toFixed(1)}%</div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">{metrics.prediction.toFixed(1)}%</div>
              <div className="text-sm text-muted-foreground">Prediction</div>
              <Progress value={metrics.prediction} className="mt-1" />
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">{metrics.automation.toFixed(1)}%</div>
              <div className="text-sm text-muted-foreground">Automation</div>
              <Progress value={metrics.automation} className="mt-1" />
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">{metrics.efficiency.toFixed(1)}%</div>
