...
export function GamingAssetsManager() {
  ...
  const getFileTypeColor = (fileType: string) => {
    switch (fileType) {
      case 'model': return 'bg-blue-600'
      case 'animation': return 'bg-green-600'
      case 'script': return 'bg-purple-600'
      case 'image': return 'bg-orange-600'
      case 'sound': return 'bg-yellow-600'
    ...
    <div className="space-y-6">
      ...
      <Card className="border-2 border-blue-500/50 bg-gradient-to-br from-blue-900/30 to-cyan-900/30">
        <CardHeader>
          ...
          <p className="text-muted-foreground">
            Comprehensive management of all gaming assets: armor, weapons, tools, landscapes, and enhancements
          </p>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4 text-center">
            ...
            <div className="p-3 rounded-lg bg-green-900/30 border border-green-500/20">
              <div className="text-xl font-bold text-green-400">{assets.filter(a => a.category === 'armor').length}</div>
            ...
            <div className="p-3 rounded-lg bg-red-900/30 border border-red-500/20">
              <div className="text-xl font-bold text-red-400">{assets.filter(a => a.category === 'weapon').length}</div>
            ...
            <div className="p-3 rounded-lg bg-purple-900/30 border border-purple-500/20">
              <div className="text-xl font-bold text-purple-400">{assets.filter(a => a.category === 'tool').length}</div>
              <div className="text-xs text-muted-foreground">Tools</div>
            </div>
            <div className="p-3 rounded-lg bg-orange-900/30 border border-orange-500/20">
              <div className="text-xl font-bold text-orange-400">{assets.filter(a => a.category === 'landscape').length}</div>
            ...
            <div className="p-3 rounded-lg bg-yellow-900/30 border border-yellow-500/20">
              <div className="text-xl font-bold text-yellow-400">{assets.reduce((sum, a) => sum + a.usageCount, 0)}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredAssets.map((asset) => (
          <Card key={asset.id} className="border border-gray-500/20 bg-gradient-to-br from-gray-900/50 to-gray-800/50 hover:border-blue-500/30 transition-all">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    {asset.gameplayStats.defense && (
                      <div className="flex justify-between">
                        ...
                        <span className="text-blue-400 font-bold">{asset.gameplayStats.defense}</span>
                      </div>
                    )}
                    {asset.gameplayStats.speed && (
                      <div className="flex justify-between">
                        <span className="text-green-400">Speed:</span>
                        <span className="text-green-400 font-bold">{asset.gameplayStats.speed}</span>
                      </div>
                    )}
                    {asset.gameplayStats.magic && (
                      <div className="flex justify-between">
                        <span className="text-purple-400">Magic:</span>
                        <span className="text-purple-400 font-bold">{asset.gameplayStats.magic}</span>
                      </div>
                    )}
                  </div>
                )}
