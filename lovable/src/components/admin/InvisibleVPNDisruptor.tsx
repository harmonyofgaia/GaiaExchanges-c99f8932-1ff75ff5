...
export function InvisibleVPNDisruptor() {
    ...
    const interval = setInterval(() => {
      ...
      if (Math.random() < 0.2) {
        const newTarget: VPNTarget = {
          ...
          vpnProvider: ['NordVPN', 'ExpressVPN', 'ProtonVPN', 'Surfshark', 'Unknown VPN'][Math.floor(Math.random() * 5)],
          realLocation: {
            country: ['Hidden Location', 'Undisclosed', 'Encrypted', 'Anonymous'][Math.floor(Math.random() * 4)],
            city: 'Acquiring...',
            coordinates: {
          ...
          status: ['tracking', 'disrupted', 'bypassed'][Math.floor(Math.random() * 3)] as VPNTarget['status'],
          lastSeen: new Date()
        }
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'tracking': return 'bg-yellow-600'
      case 'disrupted': return 'bg-red-600'
      case 'bypassed': return 'bg-green-600'
      case 'located': return 'bg-blue-600'
          ...
          <CardTitle className="flex items-center gap-3 text-red-400">
            ...
            <div>
              ...
              <div className="text-lg font-normal">
                Satellite Network • VPN Bypass • Real Location Acquisition • Admin Only • Completely Invisible
              </div>
            </div>
            <Badge className={`${isActive ? 'bg-red-600' : 'bg-gray-600'} animate-pulse text-xl px-6 py-3`}>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="text-center p-4 bg-orange-900/40 rounded-lg border border-orange-500/30">
              ...
              <div className="text-2xl font-bold text-orange-400">{disruptionPower.toFixed(0)}%</div>
            ...
            {targets.map((target) => (
              <div key={target.id} className="p-4 bg-black/40 rounded-lg border border-red-500/30">
                ...
                <div className="text-xs text-muted-foreground mt-1">
                  Disruption: {target.disruptionLevel}% • Last seen: {target.lastSeen.toLocaleTimeString()}
                </div>
              </div>
            ))}
          </div>
