...
export function PsychohistoricalEngine() {
  ...
  const generateNewIdeas = () => {
    ...
    setTimeout(() => {
      ...
      setSystemStatus(prev => ({ ...prev, isGenerating: false, dailyIdeas: prev.dailyIdeas + 1 }))
  ...
  const getStreamIcon = (type: string) => {
    switch (type) {
      case 'environmental': return <TreePine className="h-4 w-4" />
  ...
  const getStreamColor = (type: string) => {
    switch (type) {
      ...
      case 'economic': return 'border-blue-500/50 text-blue-400'
      case 'social': return 'border-purple-500/50 text-purple-400'
      case 'technological': return 'border-yellow-500/50 text-yellow-400'
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'waste_reduction': return <Wind className="h-4 w-4" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="border-green-500/20">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-green-400">Global Data Scanning</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <CardContent>
          <div className="space-y-4">
            {dataStreams.map((stream) => (
              <div key={stream.id} className="flex items-center justify-between p-3 rounded-lg bg-black/20 border border-gray-500/20">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg bg-black/30 border ${getStreamColor(stream.type)}`}>
            ...
            {predictionModels.map((model) => (
              <div key={model.id} className="p-4 rounded-lg bg-black/20 border border-gray-500/20">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <Badge variant="outline" className={`${getStatusColor(model.status)} border-current`}>
                    {model.status}
                  </Badge>
                </div>
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <span className="font-bold text-purple-400">{idea.resourceRequirement}</span>
                      </div>
                      <div>
                  ...
                  <div className="text-right ml-4">
                    <Badge variant="outline" className={
                      idea.status === 'approved' ? 'border-green-500/50 text-green-400' :
                      idea.status === 'reviewed' ? 'border-blue-500/50 text-blue-400' :
                      'border-yellow-500/50 text-yellow-400'
                    }>
                      {idea.status}
                    </Badge>
                    <div className="text-xs text-muted-foreground mt-2">
                      Generated {Math.floor((Date.now() - idea.generatedAt.getTime()) / 1000 / 60)}m ago
                    </div>
                  </div>
                </div>
