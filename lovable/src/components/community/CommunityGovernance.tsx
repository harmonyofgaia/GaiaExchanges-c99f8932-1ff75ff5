...
export function CommunityGovernance() {
  const [proposals, setProposals] = useState<Proposal[]>([
    {
      ...
      description: 'Proposal to increase GAiA earning rate from 2 to 3 tokens per km for bike activities',
      type: 'tokenomics',
      status: 'voting',
      votesFor: 1247,
      votesAgainst: 423,
    ...
    setProposals(proposals.map(proposal => 
      proposal.id === proposalId 
        ? { 
            ...
            votesAgainst: !support ? proposal.votesAgainst + userVotingPower : proposal.votesAgainst,
  ...
  const getTypeColor = (type: Proposal['type']) => {
    switch (type) {
      ...
      case 'tokenomics': return 'bg-green-600'
      case 'governance': return 'bg-purple-600'
      case 'partnership': return 'bg-orange-600'
  ...
  const getStatusIcon = (status: Proposal['status']) => {
    switch (status) {
      case 'voting': return <Vote className="h-4 w-4 text-blue-400" />
      case 'passed': return <CheckCircle className="h-4 w-4 text-green-400" />
      case 'rejected': return <CheckCircle className="h-4 w-4 text-red-400" />
      case 'implemented': return <Gavel className="h-4 w-4 text-purple-400" />
    }
  }
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-muted-foreground">
                            Proposed by: {proposal.proposedBy} • Required: {proposal.requiredTokens} GAiA to propose
                          </div>
                        </div>
                      ...
                      <div className="space-y-4">
                        ...
                        {hasVoted && (
                          <div className="text-center p-2 bg-blue-900/20 rounded border border-blue-500/20">
                            <span className="text-blue-400">✅ You have voted on this proposal</span>
                          </div>
                        )}

                        {proposal.status === 'passed' && (
                          <div className="text-center p-2 bg-green-900/20 rounded border border-green-500/20">
                            <span className="text-green-400">🎉 Proposal Passed - Implementation Pending</span>
                          </div>
                        )}
                      </div>
                    </CardContent>
