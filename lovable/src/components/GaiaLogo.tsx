  ...
  return (
    ...
    >
      <div className="relative">
        ...
        {/* Orbiting Elements */}
        <div className="absolute -top-1 -right-1 text-green-400 text-xs animate-spin">⚡</div>
        <div className="absolute -bottom-1 -left-1 text-blue-400 text-xs animate-pulse">💎</div>
      </div>

      {showText && (
        <div className="flex flex-col">
          <span className="font-bold text-green-400 leading-tight">{GAIA_BRANDING.NAME}</span>
          <span className="text-xs text-green-300 opacity-80 leading-tight">Web3 Exchange</span>
        </div>
      )}
    </div>
  );
