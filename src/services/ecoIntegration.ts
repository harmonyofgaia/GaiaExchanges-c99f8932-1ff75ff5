...
class EcoIntegrationService {
  ...
  private generateNewMission() {
    const missionTypes: EcoMission['type'][] = [
      'bike_ride', 'tree_planting', 'recycling', 'green_energy', 'ocean_cleanup'
    ]
    ...
    const newMission: EcoMission = {
      ...
      deadline: template.hasDeadline ? new Date(Date.now() + Math.random() * 14 * 24 * 60 * 60 * 1000) : undefined
    }
  ...
  recordBikeActivity(distance: number, duration: number): BikeActivity {
    ...
    console.log(`🚴 GAIA Bike activity recorded: ${distance}km, ${carbonSaved.toFixed(2)}kg CO2 saved`)
...
export function useEcoIntegration() {
  ...
  return {
    ...
    updateMissionProgress: (id: string, progress: number) => ecoIntegration.updateMissionProgress(id, progress),
    completeMission: (id: string) => ecoIntegration.completeMission(id),
    recordBikeActivity: (distance: number, duration: number) => ecoIntegration.recordBikeActivity(distance, duration)
  }
}