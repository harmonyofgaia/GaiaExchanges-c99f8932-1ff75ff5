...
class AdminDominationService {
  ...
  async executeRemoteOverride(override: {
    ...
    if (override.biometricAuth && !await this.verifyBiometricAuth(override.adminId, override.biometricAuth)) {
  ...
  async registerBiometricData(adminId: string, biometricData: BiometricData): Promise<void> {
    ...
    console.log('👁️ Biometric Data Registered:', {
      adminId,
      methods: Object.keys(biometricData).filter(k => k !== 'verified' && biometricData[k as keyof BiometricData])
    })
