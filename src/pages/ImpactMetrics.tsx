...
export default function ImpactMetrics() {
  ...
  useEffect(() => {
    ...
    const mockGlobalMetrics: GlobalMetric[] = [
      { title: 'Global Carbon Offset', current: 2847291, target: 5000000, unit: 'tonnes CO₂', color: 'bg-green-600' },
      { title: 'Trees Planted', current: 1234567, target: 2000000, unit: 'trees', color: 'bg-emerald-600' },
      { title: 'Ocean Plastic Removed', current: 45623, target: 100000, unit: 'kg', color: 'bg-blue-600' },
      { title: 'Renewable Energy', current: 78934, target: 150000, unit: 'MWh', color: 'bg-yellow-600' }
    ]
  ...
  const achievements = [
    { title: 'Carbon Hero', description: 'Offset 200kg CO₂ this month', icon: '🌱', earned: true },
    { title: 'Water Guardian', description: 'Saved 1000+ liters', icon: '💧', earned: true },
    { title: 'Energy Pioneer', description: 'Generated 500kWh clean energy', icon: '⚡', earned: false },
    { title: 'Eco Influencer', description: 'Inspired 10+ community members', icon: '🌟', earned: true },
  ]
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
            {personalImpact.map((impact, index) => (
              <Card key={impact.category} className="bg-gradient-to-br from-gray-900/50 to-black/50 border-gray-700/30">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className={impact.color}>
                        {impact.icon}
                      </div>
                  ...
                  <div className="mt-4">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-green-400">
                        +{(impact.value * impact.change / 100).toFixed(1)} {impact.unit} this {selectedTimeframe}
                      </span>
                    </div>
                  </div>
                </CardContent>
            ...
            <Card className="bg-yellow-900/20 border-yellow-500/30">
              ...
              <CardContent className="space-y-3">
                {achievements.map(achievement => (
                  <div key={achievement.title} className={`flex items-center gap-3 p-2 rounded-lg ${
                    achievement.earned ? 'bg-green-900/20 border border-green-500/30' : 'bg-gray-900/20 border border-gray-600/30'
                  }`}>
