...
export default function MobileDashboard() {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, type: 'reward', message: 'You earned 50 GAIA tokens!', time: '2m ago' },
    { id: 2, type: 'achievement', message: 'New badge unlocked: Eco Warrior', time: '1h ago' },
    { id: 3, type: 'reminder', message: 'Daily check-in available', time: '3h ago' }
  ])

  const quickActions = [
    { id: 'check-in', label: '✅ Daily Check-in', color: 'bg-green-600', action: () => handleDailyCheckIn() },
    { id: 'scan-receipt', label: '🧾 Scan Receipt', color: 'bg-blue-600', action: () => handleScanReceipt() },
    { id: 'plant-tree', label: '🌱 Plant Tree', color: 'bg-emerald-600', action: () => handlePlantTree() },
    { id: 'share-impact', label: '📤 Share Impact', color: 'bg-purple-600', action: () => handleShareImpact() }
  ]
            ...
            <div className="grid grid-cols-2 gap-3">
              {quickActions.map((action) => (
                ...
                >
                  ...
                  <span className="text-xs">{action.label.substring(action.label.indexOf(' ') + 1)}</span>
                </Button>
              ))}
            </div>
          </CardContent>
