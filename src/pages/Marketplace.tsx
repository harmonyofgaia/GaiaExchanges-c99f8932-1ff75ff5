...
export default function Marketplace() {
  ...
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      case 'Divine': return 'bg-gradient-to-r from-yellow-400 to-white text-black'
      case 'Mythic': return 'bg-gradient-to-r from-pink-500 to-purple-500'
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'animal': return <Heart className="h-5 w-5" />
      case 'nft': return <Gem className="h-5 w-5" />
    ...
    <div className="container mx-auto px-4 py-8">
      <Card className="border-blue-500/30 bg-gradient-to-r from-blue-900/20 to-green-900/20 mb-8">
        <CardHeader>
          ...
          <p className="text-center text-xl text-muted-foreground">
            Trade NFTs, invest in animal rescue, buy land for restoration, and discover exclusive tools
          </p>
          <div className="flex justify-center gap-4 flex-wrap mt-4">
            <Badge className="bg-blue-600">ITEMS: {marketStats.totalItems}</Badge>
            <Badge className="bg-green-600">VALUE: ${marketStats.totalValue.toLocaleString()}</Badge>
            <Badge className="bg-purple-600 animate-pulse">ANIMALS SAVED: {marketStats.animalsSaved}</Badge>
      ...
      <Tabs defaultValue="featured" className="w-full">
                  ...
                  {item.type === 'animal' && item.investmentGoal && (
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span>${item.currentInvestment?.toLocaleString()} / ${item.investmentGoal.toLocaleString()}</span>
                      </div>
                      <div className="w-full bg-gray-700 rounded-full h-2">
                        <div 
                          className="bg-green-500 h-2 rounded-full" 
                          style={{ width: `${((item.currentInvestment || 0) / item.investmentGoal) * 100}%` }}
                        ></div>
                      </div>
                      {item.liveTracking && (
        ...
        <TabsContent value="animals" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.filter(item => item.type === 'animal').map((item) => (
              <Card key={item.id} className="border-green-500/30 bg-green-900/20">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    {item.liveTracking && <Badge className="bg-blue-600 animate-pulse">LIVE</Badge>}
                  </div>
                </CardHeader>
                <CardContent className="space-y-4">
                  <p className="text-sm text-muted-foreground">{item.description}</p>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-bold">
                          ${item.currentInvestment?.toLocaleString()} / ${item.investmentGoal.toLocaleString()}
                        </span>
                      </div>
                      ...
                      <div className="text-center text-sm text-green-400">
                        {Math.round(((item.currentInvestment || 0) / item.investmentGoal) * 100)}% funded
                      </div>
                    </div>
                  )}
        ...
        <TabsContent value="nfts" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredItems.filter(item => item.type === 'nft').map((item) => (
        ...
        <TabsContent value="tools" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.filter(item => item.type === 'tool').map((item) => (
        ...
        <TabsContent value="land" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredItems.filter(item => item.type === 'land').map((item) => (
