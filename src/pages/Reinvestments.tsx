  ...
  const reinvestmentProjects = [
    {
      ...
      description:
        "Personal project: Restoring coral reefs with balanced underwater audio signals to attract marine life and recover ecosystems",
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-green-400">
            Harmony of Gaia - Reinvestments Overview
          </h1>
          <p className="text-muted-foreground">
            Complete transparency: 5% coral reef restoration, 90% project reinvestment, 10%
            community rewards
          </p>
        </div>
        <Badge className="bg-green-600 text-white px-4 py-2">Live Data • Updates Every 5s</Badge>
      </div>
        ...
        <CardContent className="space-y-4">
          <div className="text-center">
            ...
            <p className="text-lg text-cyan-300 mb-4">
              Personal commitment: Revolutionary underwater audio technology helping coral reefs
              recover and attract marine life back to damaged ecosystems
            </p>
          </div>
          ...
          <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-lg p-4">
            ...
            <p className="text-sm text-cyan-300">
              This is where 5% of ALL GAiA token burns are automatically sent to fund real-world
              coral reef restoration through Sound Riffs Re Grau dio technology.
            </p>
          </div>
      ...
      <Card className="border-green-500/20">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-green-400">
            ...
            Environmental Reinvestment Projects - Updated Allocation (5% Coral / 90% Project / 10%
            Community)
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {reinvestmentProjects.map((project) => (
              <Card
                key={project.id}
                className={`${project.id === 1 ? "bg-cyan-900/20 border-cyan-500/30" : project.category === "Community" ? "bg-blue-900/20 border-blue-500/30" : "bg-muted/20 border-border/30"}`}
              >
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3
                      className={`font-semibold ${project.id === 1 ? "text-cyan-400" : project.category === "Community" ? "text-blue-400" : ""}`}
                    >
                      {project.id === 1 && "🪸 "}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                    <div>
                      ...
                      <div
                        className={`font-semibold ${project.id === 1 ? "text-cyan-400" : project.category === "Community" ? "text-blue-400" : "text-green-400"}`}
                      >
                    ...
                    <div>
                      ...
                      <div
                        className={`font-semibold ${project.id === 1 ? "text-cyan-400" : project.category === "Community" ? "text-blue-400" : "text-primary"}`}
                      >
                        {project.impact}
                      </div>
                    </div>
                  ...
                  {project.id === 1 && (
                    <div className="mt-3 space-y-2">
                      <div className="p-3 bg-cyan-500/10 rounded border border-cyan-500/20">
                        <p className="text-sm text-cyan-300">
                          🎵 Personal project: {project.burnContribution}% of all GAiA token burns
                          automatically fund coral reef restoration through innovative sound
                          technology.
                        </p>
                      </div>
                      <div className="bg-black/30 p-2 rounded">
                        ...
                        <div className="font-mono text-xs text-cyan-300 break-all">
                          {project.walletAddress === GAIA_TOKEN.WALLET_ADDRESS
                            ? "Main Community Wallet"
                            : project.walletAddress === GAIA_TOKEN.GREEN_INVESTMENTS_WALLET
                              ? "Green Investments Wallet"
                              : project.walletAddress === GAIA_TOKEN.COMMUNITY_VAULT_WALLET
                                ? "Community Vault Wallet"
                                : project.walletAddress === GAIA_TOKEN.ANIMAL_WELFARE_WALLET
      ...
      <Card className="bg-gradient-to-r from-green-900/20 to-blue-900/20 border-primary/20">
        <CardContent className="p-6">
          <div className="text-center space-y-4">
            ...
            <p className="text-muted-foreground">
              Join the movement to create the world's most trusted and environmentally conscious
              crypto exchange with 90% project reinvestment
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Button
                variant="outline"
                className="border-blue-500/30 text-blue-400 hover:bg-blue-900/20 flex items-center gap-2"
              >
              ...
              <Button
                variant="outline"
                className="border-purple-500/30 text-purple-400 hover:bg-purple-900/20 flex items-center gap-2"
              >
          ...
          <div className="flex items-center gap-3 text-center justify-center">
            ...
            <div>
              <h4 className="font-semibold text-green-400">
                🌍 Harmony of Gaia Trust Network - Official Addresses Connected
              </h4>
              <p className="text-sm text-muted-foreground">
                Every token verified • Every transaction transparent • Every reinvestment tracked •
                90% project reinvestment • Building the future of sustainable finance
              </p>
              <div className="mt-2 text-xs text-green-400">
                Main Community Wallet: {GAIA_TOKEN.WALLET_ADDRESS} | Contract:{" "}
                {GAIA_TOKEN.CONTRACT_ADDRESS}
              </div>
            </div>
          </div>
