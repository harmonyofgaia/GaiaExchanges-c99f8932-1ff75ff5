...
export default function EcoAvatarGaiaSoulSystem() {
  ...
  const startMeditation = (element: string) => {
    ...
    setTimeout(() => {
      if (avatar) {
        ...
        const attributeKey = `${element}${element === 'earth' ? 'Connection' : 
          element === 'water' ? 'Harmony' : 
          element === 'air' ? 'Alignment' : 'Energy'}` as keyof GaiaSoulAttributes
        
        updatedAvatar.attributes[attributeKey] = Math.min(100, updatedAvatar.attributes[attributeKey] + 5)
  ...
  const elements = [
    { name: 'earth', label: 'Earth Connection', description: 'Deepen your connection with the Earth' },
    { name: 'water', label: 'Water Harmony', description: 'Flow in harmony with water cycles' },
    { name: 'air', label: 'Air Alignment', description: 'Align with the breath of the planet' },
    { name: 'fire', label: 'Fire Energy', description: 'Channel the transformative power of fire' },
    { name: 'spirit', label: 'Spirit Balance', description: 'Balance your spiritual essence' }
  ]
      ...
      <Card className="border-green-500/30 bg-gradient-to-r from-green-900/20 to-blue-900/20">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-yellow-400">{avatar.environmental_impact}</div>
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {elements.map((element) => {
              ...
              return (
                <Card key={element.name} className="border-muted/20 hover:border-purple-500/40 transition-colors">
                  <CardContent className="p-4 text-center">
                    ...
                    <Button
                      ...
                      className="w-full"
                    >
                      {meditating && selectedElement === element.name ? 'Meditating...' : 'Meditate'}
                    </Button>
                  </CardContent>
                </Card>
              )
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {avatar.spiritual_achievements.map((achievement, index) => (
              <div key={index} className="flex items-center gap-3 p-3 rounded-lg border border-yellow-500/30 bg-yellow-900/20">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {avatar.unlocked_features.map((feature, index) => (
              <div key={index} className="flex items-center gap-3 p-3 rounded-lg border border-green-500/30 bg-green-900/20">
        ...
        <CardContent>
          <div className="text-center space-y-4">
            <div className="w-32 h-32 mx-auto relative">
              ...
              <div className="absolute inset-2 rounded-full border-2 border-green-500/30 animate-pulse" style={{ animationDelay: '0.5s' }}></div>
              <div className="absolute inset-4 rounded-full border border-blue-500/30 animate-pulse" style={{ animationDelay: '1s' }}></div>
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Your soul is in harmony with Gaia's frequencies. Continue your spiritual journey 
                to unlock deeper connections with the natural world.
              </p>
            </div>
