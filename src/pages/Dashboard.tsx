...
export default function Dashboard() {
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      case 'contribution': return <Heart className="h-4 w-4 text-pink-400" />
      case 'gaming': return <Gamepad2 className="h-4 w-4 text-purple-400" />
      case 'nft': return <Star className="h-4 w-4 text-yellow-400" />
      case 'trading': return <TrendingUp className="h-4 w-4 text-green-400" />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Card className="border-pink-500/30 bg-gradient-to-br from-pink-900/20 to-red-900/20">
                <CardContent className="p-6 text-center">
                  ...
                  <div className="text-xs text-pink-300 mt-1">Direct impact through your contributions</div>
                </CardContent>
              </Card>
              ...
              <Card className="border-green-500/30 bg-gradient-to-br from-green-900/20 to-emerald-900/20">
                <CardContent className="p-6 text-center">
                  ...
                  <div className="text-xs text-green-300 mt-1">{userImpact.carbonOffset.toFixed(1)} tons CO2 offset</div>
                </CardContent>
              </Card>
              ...
              <Card className="border-yellow-500/30 bg-gradient-to-br from-yellow-900/20 to-orange-900/20">
                <CardContent className="p-6 text-center">
                  ...
                  <div className="text-3xl font-bold text-yellow-400">{userImpact.totalContribution.toFixed(0)}</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="text-pink-400">{((userImpact.animalsHelped / (userImpact.animalsHelped + userImpact.treesPlanted + userImpact.gamesPlayed || 1)) * 100).toFixed(0)}%</span>
                    </div>
                    <Progress value={(userImpact.animalsHelped / (userImpact.animalsHelped + userImpact.treesPlanted + userImpact.gamesPlayed || 1)) * 100} className="h-2" />
                  </div>
                  <div>
                    <div className="flex justify-between mb-2">
                      <span className="text-green-400">Environmental</span>
                      <span className="text-green-400">{((userImpact.treesPlanted / (userImpact.animalsHelped + userImpact.treesPlanted + userImpact.gamesPlayed || 1)) * 100).toFixed(0)}%</span>
                    </div>
                    <Progress value={(userImpact.treesPlanted / (userImpact.animalsHelped + userImpact.treesPlanted + userImpact.gamesPlayed || 1)) * 100} className="h-2" />
                  </div>
                  <div>
                    <div className="flex justify-between mb-2">
                      <span className="text-purple-400">Gaming Contribution</span>
                      <span className="text-purple-400">{((userImpact.gamesPlayed / (userImpact.animalsHelped + userImpact.treesPlanted + userImpact.gamesPlayed || 1)) * 100).toFixed(0)}%</span>
                    </div>
                    <Progress value={(userImpact.gamesPlayed / (userImpact.animalsHelped + userImpact.treesPlanted + userImpact.gamesPlayed || 1)) * 100} className="h-2" />
                  </div>
                </div>
              </CardContent>
            </Card>
                ...
                <Card key={project.id} className={`${getUrgencyColor(project.urgency)} border-2`}>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      <Badge className={project.urgency === 'high' ? 'bg-red-600' : project.urgency === 'medium' ? 'bg-yellow-600' : 'bg-green-600'}>
                  ...
                  <CardContent className="space-y-4">
                    <div>
                      <div className="flex justify-between mb-2">
                        ...
                        <span className="text-sm font-bold">{project.currentFunding.toLocaleString()} / {project.goalFunding.toLocaleString()} GAiA</span>
                      </div>
                      <Progress value={(project.currentFunding / project.goalFunding) * 100} className="h-3" />
                    </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  {recentActivities.map((activity) => (
                    <div key={activity.id} className="flex items-start gap-4 p-4 rounded-lg bg-black/30 border border-white/10">
                      <div className="mt-1">
                      ...
                      <div className="text-right">
                        <div className="font-bold text-yellow-400">{activity.tokens.toFixed(2)} GAiA</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Card className="border-yellow-500/30 bg-gradient-to-br from-yellow-900/20 to-orange-900/20">
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">Helped save {userImpact.animalsHelped} animals</p>
              ...
              <Card className="border-green-500/30 bg-gradient-to-br from-green-900/20 to-emerald-900/20">
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">Planted {userImpact.treesPlanted} trees</p>
              ...
              <Card className="border-purple-500/30 bg-gradient-to-br from-purple-900/20 to-indigo-900/20">
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">Contributing through {userImpact.gamesPlayed} games</p>
