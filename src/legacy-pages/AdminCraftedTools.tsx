...
export default function AdminCraftedTools() {
  ...
  const addToMarketplace = (itemId: string) => {
    ...
    setCraftedItems(prev => prev.map(item => 
      item.id === itemId 
  ...
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      case 'Divine': return 'bg-gradient-to-r from-yellow-400 to-white'
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'upgrade': return <Zap className="h-5 w-5" />
      case 'power': return <Gem className="h-5 w-5" />
    ...
    <div className="container mx-auto px-4 py-8">
      <Card className="border-yellow-500/30 bg-gradient-to-r from-yellow-900/20 to-purple-900/20 mb-8">
        <CardHeader>
          ...
          <div className="flex justify-center gap-4 flex-wrap mt-4">
            <Badge className="bg-yellow-600 animate-pulse">ADMIN POWER: {Math.floor(adminPower).toLocaleString()}</Badge>
            <Badge className="bg-purple-600">TOTAL ITEMS: {craftedItems.length}</Badge>
            <Badge className="bg-green-600">ON MARKETPLACE: {craftedItems.filter(i => i.onMarketplace).length}</Badge>
          </div>
        </CardHeader>
      </Card>
      ...
      <Tabs defaultValue="all" className="w-full">
                  ...
                  {!item.onMarketplace ? (
                    <div className="space-y-2">
                      <Input
                        ...
                        onChange={(e) => setNewItemPrice(prev => ({
                          ...prev,
                    ...
                    <div className="space-y-2">
                      <div className="text-center p-2 bg-green-500/20 rounded border border-green-500/30">
                        <span className="text-green-400 font-bold">On Marketplace: ${item.marketPrice}</span>
                      </div>
                      <Button 
        ...
        <TabsContent value="marketplace" className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="text-center p-4 bg-blue-500/20 rounded">
                  <div className="text-2xl font-bold text-blue-400">
                    ${craftedItems.filter(i => i.onMarketplace).reduce((sum, i) => sum + i.marketPrice, 0)}
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {craftedItems.filter(item => item.onMarketplace).map((item) => (
