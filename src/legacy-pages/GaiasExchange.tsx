...
const GaiasExchange = () => {
  ...
  const downloadApp = (platform: string) => {
    toast.success(`📱 Downloading Gaia's Exchange for ${platform}!`, {
      description: 'The most trusted crypto exchange app - now available on all platforms!',
      duration: 5000
    })
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <Card className="bg-gradient-to-br from-green-900/30 to-cyan-900/30 border-2 border-green-500/50">
            <CardContent className="pt-4 text-center">
              ...
              <div className="text-2xl font-bold text-green-400">${marketData.gaiaPrice.toFixed(6)}</div>
              <div className="text-xs text-green-300 flex items-center justify-center gap-1">
                <ArrowUpRight className="h-3 w-3" />
                +{marketData.gaiaChange.toFixed(2)}%
              </div>
              <div className="text-xs text-muted-foreground mt-1">GAIA Token</div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-orange-900/30 to-yellow-900/30 border-2 border-orange-500/50">
            <CardContent className="pt-4 text-center">
              ...
              <div className="text-2xl font-bold text-orange-400">${marketData.totalVolume.toLocaleString()}</div>
          ...
          <Card className="bg-gradient-to-br from-blue-900/30 to-purple-900/30 border-2 border-blue-500/50">
            <CardContent className="pt-4 text-center">
              ...
              <div className="text-2xl font-bold text-blue-400">{marketData.totalUsers.toLocaleString()}</div>
          ...
          <Card className="bg-gradient-to-br from-purple-900/30 to-pink-900/30 border-2 border-purple-500/50">
            <CardContent className="pt-4 text-center">
              ...
              <div className="text-2xl font-bold text-purple-400">{marketData.dailyTrades.toLocaleString()}</div>
              ...
              <CardContent>
                <div className="space-y-3">
                  {tradingPairs.map((pair, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-muted/20 rounded border border-border/30 hover:bg-muted/30 transition-colors">
                      <div className="flex items-center gap-3">
                        <div className="font-bold text-white">{pair.pair}</div>
                        {pair.pair.includes('GAIA') && (
                          <Badge className="bg-green-600 text-white text-xs">HOT</Badge>
                        )}
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-white">${pair.price.toLocaleString()}</div>
                        <div className={`text-xs flex items-center gap-1 ${
                          pair.change > 0 ? 'text-green-400' : 'text-red-400'
                        }`}>
                          {pair.change > 0 ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
                          {pair.change > 0 ? '+' : ''}{pair.change}%
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm text-muted-foreground">Vol: {pair.volume}</div>
                        <Button size="sm" className="mt-1 bg-gradient-to-r from-green-600 to-blue-600 hover:from-green-700 hover:to-blue-700">
                          Trade
                        </Button>
                      </div>
                    </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" className="min-w-20">GAIA</Button>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex gap-2">
                      <Input placeholder="Amount" className="flex-1" />
                      <Button variant="outline" className="min-w-20">USDT</Button>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
                  <div className="flex justify-between text-sm mb-2">
                    ...
                    <span className="text-green-400 font-bold">1 GAIA = $0.00247</span>
                  </div>
                  <div className="flex justify-between text-sm mb-2">
                    <span>Network Fee:</span>
                    <span className="text-green-400 font-bold">0.1% (Lowest in Industry)</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Environmental Contribution:</span>
                    <span className="text-green-400 font-bold">0.05% to Coral Reefs</span>
                  </div>
                </div>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-purple-400">{appDownloadStats.totalDownloads.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm">Daily Downloads:</span>
                    <span className="font-bold text-green-400">+{appDownloadStats.dailyDownloads}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm">App Rating:</span>
                    <div className="flex items-center gap-1">
                      <Star className="h-4 w-4 text-yellow-400 fill-current" />
                      <span className="font-bold text-yellow-400">{appDownloadStats.appRating}</span>
                    </div>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm">Active Users:</span>
                    <span className="font-bold text-blue-400">{appDownloadStats.activeUsers.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm">Current Version:</span>
                    <span className="font-bold text-cyan-400">v{appDownloadStats.appVersion}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-blue-900/30 to-green-900/30 border-2 border-blue-500/50">
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground text-center mb-4">
                  Access the world's most trusted environmental crypto exchange on any device!
                </p>
                ...
                <div className="mt-6 p-4 bg-gradient-to-r from-green-900/30 to-blue-900/30 border border-green-500/20 rounded-lg">
                  <div className="text-center">
                    ...
                    <h4 className="font-bold text-green-400 mb-2">🛡️ Ultra-Secure & Trusted</h4>
                    <p className="text-xs text-muted-foreground">
                      Bank-level security, multi-factor authentication, and the highest trust score in the industry
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="bg-gradient-to-br from-green-900/30 to-cyan-900/30 border-2 border-green-500/50">
              <CardContent className="pt-6">
                <div className="text-center">
                  ...
                  <h4 className="font-bold text-green-400 mb-2">🌍 Environmental Mission</h4>
                  <p className="text-sm text-muted-foreground mb-4">
                    Every trade on Gaia's Exchange contributes to coral reef restoration and environmental projects worldwide!
                  </p>
                  <div className="text-lg font-bold text-green-400 mb-1">
                    0.05% of all trades
                  </div>
                  <div className="text-xs text-green-300">Goes directly to coral reef restoration</div>
                  
                  <div className="mt-4 p-3 bg-cyan-500/10 rounded border border-cyan-500/20">
                    <div className="text-sm font-bold text-cyan-400 mb-1">🪸 Sound Riffs Re Grau dio Project</div>
                    <div className="text-xs text-muted-foreground">
                      Revolutionary underwater audio technology restoring coral reefs worldwide
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <CardContent className="py-8">
              <div className="text-center">
                ...
                <p className="text-xl text-muted-foreground mb-6">
                  🏛️ Building the future of environmental cryptocurrency trading with unmatched security and transparency
                </p>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="p-4 bg-blue-500/10 rounded border border-blue-500/20">
                    ...
                    <div className="font-bold text-blue-400">Bank-Level Security</div>
                  ...
                  <div className="p-4 bg-green-500/10 rounded border border-green-500/20">
                    ...
                    <div className="font-bold text-green-400">24/7 Availability</div>
                    <div className="text-xs text-muted-foreground">99.99% uptime guarantee</div>
                  </div>
                  <div className="p-4 bg-purple-500/10 rounded border border-purple-500/20">
                    <Users className="h-8 w-8 text-purple-400 mx-auto mb-2" />
                    <div className="font-bold text-purple-400">Global Community</div>
                    <div className="text-xs text-muted-foreground">247K+ active traders</div>
                  </div>
                  <div className="p-4 bg-cyan-500/10 rounded border border-cyan-500/20">
                    <Heart className="h-8 w-8 text-cyan-400 mx-auto mb-2" />
                    <div className="font-bold text-cyan-400">Environmental Focus</div>
                    <div className="text-xs text-muted-foreground">Every trade helps nature</div>
                  </div>
                </div>
              </div>
            </CardContent>
