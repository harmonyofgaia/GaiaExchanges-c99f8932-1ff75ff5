...
export function UnifiedMusicPlayer() {
      ...
      if (activeMediaId && activeMediaData) {
        try {
          ...
          if (mediaData.type === 'audio') {
            ...
            setIsVisible(true)
            console.log('🎵 Unified Music Player Loaded:', mediaData.name || mediaData.original_name)
          }
        } catch (error) {
  ...
  const toggleMute = () => {
    setIsMuted(!isMuted)
    toast.info(isMuted ? '🔊 Audio unmuted' : '🔇 Audio muted', { duration: 1000 })
  }
  ...
  return (
    <>
      ...
      <Card className={`fixed bottom-4 right-4 z-50 border-primary/30 bg-background/95 backdrop-blur-md transition-all duration-300 ${
