      ...
      try {
        ...
        if (activePlaylistId) {
          ...
          if (activePlaylist) {
            ...
            if (currentTrackId) {
              const trackIndex = activePlaylist.files.findIndex((f: MediaFile) => f.id === currentTrackId)
              if (trackIndex !== -1) {
    ...
    <>
      ...
      {isVisible && (
        <div className={`fixed ${isMinimized ? 'bottom-4 right-4' : 'bottom-4 right-4'} z-50 transition-all duration-300`}>
          <Card className={`${isMinimized ? 'w-80' : 'w-96'} bg-black/95 backdrop-blur-md border-purple-500/30 shadow-2xl`}>
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <CardTitle className="text-purple-400 flex items-center gap-2">
                  {currentTrack?.type === 'video' ? <Video className="h-4 w-4" /> : <Music className="h-4 w-4" />}
                  Gaia Media Player
                </CardTitle>
              ...
              <CardContent className="pt-0">
                ...
                <div className="mb-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {currentPlaylist?.name} • {currentIndex + 1} of {currentPlaylist?.files.length || 0}
                  </p>
                </div>
                      ...
                      <TabsContent value="current" className="space-y-2 max-h-32 overflow-y-auto">
                        {currentPlaylist?.files.map((file, index) => (
                          ...
                          >
                            ...
                            {file.type === 'video' ? <Video className="h-3 w-3" /> : <Music className="h-3 w-3" />}
                          </div>
                        ))}
                        ...
                        {playlists.map(playlist => (
                          ...
                          >
                            <div>
                              ...
                              <div className="text-xs text-muted-foreground">{playlist.files.length} files</div>
                            </div>
