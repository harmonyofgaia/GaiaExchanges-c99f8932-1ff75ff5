...
export function GlobalLeaderboard() {
  ...
  const getRankBg = (rank: number) => {
    switch (rank) {
      ...
      case 2:
        return "bg-gradient-to-r from-gray-700/30 to-slate-700/30 border-gray-400/50";
      case 3:
        return "bg-gradient-to-r from-amber-800/30 to-orange-800/30 border-amber-600/50";
      default:
        return "bg-gradient-to-r from-gray-800/30 to-gray-700/30 border-gray-600/30";
    }
  };
        ...
        <CardContent>
          <div className="space-y-3">
            {leaderboard.map((entry) => (
              <Card
                key={entry.rank}
                className={`${getRankBg(entry.rank)} relative overflow-hidden`}
              >
