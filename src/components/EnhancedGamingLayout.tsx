...
export function EnhancedGamingLayout() {
  ...
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      case 'Easy': return 'from-green-600 to-emerald-600'
      case 'Medium': return 'from-yellow-600 to-orange-600'
      case 'Hard': return 'from-red-600 to-pink-600'
      case 'Extreme': return 'from-purple-600 to-violet-600'
  ...
  return (
    <div className="space-y-8">
          ...
          >
            ...
            <CardHeader>
              ...
              <div className="flex items-center gap-2">
                <Badge className={`bg-gradient-to-r ${getDifficultyColor(project.difficulty)} text-white`}>
                  {project.difficulty}
                </Badge>
                <Badge 
                  className={
                    project.status === 'Active' ? 'bg-green-600' :
                    project.status === 'Coming Soon' ? 'bg-yellow-600' : 'bg-red-600'
                  }
                >
                  {project.status}
                </Badge>
      ...
      {/* Ancient Creatures Level Indicator */}
