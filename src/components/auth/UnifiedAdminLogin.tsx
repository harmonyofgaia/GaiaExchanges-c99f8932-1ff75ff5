...
export function UnifiedAdminLogin() {
  ...
  useEffect(() => {
    const checkTrustedIP = async () => {
      try {
        ...
        console.log('🔒 Admin Login - Current User IP:', userIP)
        ...
        console.log('🔒 Checking against trusted IPs for admin access')

        const isTrusted = trustedIPs.includes(userIP) || 
                         window.location.hostname === 'localhost'

        setIsTrustedIP(isTrusted)
        console.log('🔒 ADMIN ACCESS VERIFICATION:', isTrusted ? 'TRUSTED IP - SHOWING SUPREME LOGIN' : 'REGULAR IP - SHOWING GAIA LOGIN')

      } catch (error) {
        console.log('🔐 IP Protection Active - Using localhost fallback for admin')
        const isLocalhost = window.location.hostname === 'localhost'
        setIsTrustedIP(isLocalhost)
        console.log('🔒 Admin localhost check result:', isLocalhost)
      } finally {
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Supreme admin credentials (keeping current functionality)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-black via-gray-900 to-green-900">
        <div className="text-center space-y-4">
          ...
          <p className="text-green-400 font-medium">🔒 IP Verification in Progress...</p>
          <p className="text-green-300 text-sm">Quantum security protocols active</p>
        </div>
      </div>
    )
  }
  ...
  if (isTrustedIP) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-black via-red-900/20 to-yellow-900/20 flex items-center justify-center p-6">
          ...
          <CardContent>
            <form onSubmit={handleAdminLogin} className="space-y-4">
              ...
              <Button 
                ...
                className="w-full bg-gradient-to-r from-red-600 to-yellow-600 hover:from-red-700 hover:to-yellow-700 text-white font-bold py-3"
              >
            ...
            <div className="mt-6 p-4 bg-gradient-to-r from-red-900/30 to-yellow-900/30 border border-red-500/20 rounded-lg">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-green-900/20 to-blue-900/20 flex items-center justify-center p-6">
