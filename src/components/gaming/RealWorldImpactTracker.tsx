...
export function RealWorldImpactTracker() {
  ...
  const getProjectIcon = (type: string) => {
    switch (type) {
      case 'forest': return <TreePine className="h-5 w-5 text-green-400" />
      case 'ocean': return <Waves className="h-5 w-5 text-blue-400" />
      case 'wildlife': return <Fish className="h-5 w-5 text-purple-400" />
      case 'renewable': return <Sun className="h-5 w-5 text-yellow-400" />
  ...
  const getProjectColor = (type: string) => {
    switch (type) {
      case 'forest': return 'from-green-900/30 to-emerald-900/30 border-green-500/30'
      case 'ocean': return 'from-blue-900/30 to-cyan-900/30 border-blue-500/30'
      case 'wildlife': return 'from-purple-900/30 to-pink-900/30 border-purple-500/30'
      case 'renewable': return 'from-yellow-900/30 to-orange-900/30 border-yellow-500/30'
      default: return 'from-green-900/30 to-emerald-900/30 border-green-500/30'
    }
  }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-green-900/30 to-emerald-900/30 border border-green-500/30">
          <CardContent className="pt-6 text-center">
            ...
            <div className="text-3xl font-bold text-green-400">{metrics.treesPlanted.toLocaleString()}</div>
            <div className="text-sm text-green-300">Trees Planted</div>
            <div className="text-xs text-muted-foreground mt-1">+{Math.floor(Math.random() * 5) + 1} today</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-blue-900/30 to-cyan-900/30 border border-blue-500/30">
          <CardContent className="pt-6 text-center">
            ...
            <div className="text-3xl font-bold text-blue-400">{metrics.oceanCleanup.toLocaleString()}</div>
            <div className="text-sm text-blue-300">kg Ocean Cleaned</div>
            <div className="text-xs text-muted-foreground mt-1">+{Math.floor(Math.random() * 50) + 10}kg today</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-orange-900/30 to-red-900/30 border border-orange-500/30">
          <CardContent className="pt-6 text-center">
            ...
            <div className="text-3xl font-bold text-orange-400">{metrics.carbonOffset.toFixed(1)}</div>
            <div className="text-sm text-orange-300">Tons CO₂ Offset</div>
            <div className="text-xs text-muted-foreground mt-1">+{(Math.random() * 2).toFixed(1)} today</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-purple-900/30 to-pink-900/30 border border-purple-500/30">
          <CardContent className="pt-6 text-center">
            ...
            <div className="text-xs text-muted-foreground mt-1">+{Math.floor(Math.random() * 3) + 1} today</div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="text-center p-4 bg-green-900/30 rounded border border-green-500/20">
              <TreePine className="h-8 w-8 text-green-400 mx-auto mb-2" />
              <div className="text-2xl font-bold text-green-400">{userContribution.treesContributed}</div>
            ...
            <div className="text-center p-4 bg-blue-900/30 rounded border border-blue-500/20">
              ...
              <div className="text-2xl font-bold text-blue-400">{userContribution.carbonOffset}</div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {projects.map((project) => (
              <Card key={project.id} className={`bg-gradient-to-br ${getProjectColor(project.type)} border`}>
                <CardHeader>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="p-3 bg-black/30 rounded border border-border/30">
                    <div className="text-sm font-semibold text-green-400 mb-1">🌟 Real Impact Achieved:</div>
