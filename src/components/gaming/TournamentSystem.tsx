...
export function TournamentSystem() {
  const [selectedTab, setSelectedTab] = useState<'browse' | 'active' | 'brackets' | 'history'>('browse')
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'battle_royale': return <Target className="h-4 w-4" />
      case 'league': return <Crown className="h-4 w-4" />
      ...
      {selectedTab === 'browse' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tournaments.map((tournament) => (
            <Card key={tournament.id} className="bg-gradient-to-br from-gray-900/50 to-purple-900/30 border border-purple-500/30">
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Badge className={`${getTournamentStatusColor(tournament.status)} text-white text-xs`}>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-yellow-400 font-bold">{tournament.prizePool.toLocaleString()} GAIA</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Entry Fee:</span>
                ...
                <Button 
                  ...
                  disabled={tournament.participants >= tournament.maxParticipants || tournament.status === 'active'}
                  className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700"
                >
                  {tournament.status === 'registration' ? 'Join Tournament' : 
                   tournament.status === 'active' ? 'In Progress' : 'Coming Soon'}
                </Button>
              </CardContent>
            </Card>
          ))}
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div className="p-3 bg-blue-900/30 rounded">
                  <div className="text-lg font-bold text-blue-400">{activeTournament.participants}</div>
                  <div className="text-xs text-muted-foreground">Participants</div>
                </div>
                <div className="p-3 bg-yellow-900/30 rounded">
                  <div className="text-lg font-bold text-yellow-400">{activeTournament.prizePool.toLocaleString()}</div>
              ...
              <div className="space-y-4">
                ...
                {matches.map((match) => (
                  <div key={match.id} className="p-4 bg-black/30 rounded-lg border border-border/30">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-center">
                        ...
                        <Badge className={`${
                          ...
                          match.status === 'upcoming' ? 'bg-yellow-600' : 'bg-green-600'
                        } text-white text-xs`}>
