...
export function PublicTransportRewards() {
  ...
  const [recentTrips] = useState([
    { id: '1', type: 'metro', route: 'Green Line', distance: 12.5, tokens: 25, date: '2024-02-08' },
    { id: '2', type: 'bus', route: 'Route 42', distance: 8.3, tokens: 16, date: '2024-02-07' },
    { id: '3', type: 'train', route: 'Regional Express', distance: 45.2, tokens: 90, date: '2024-02-06' }
  ])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const transportBonus = basePoints * (transportMultipliers[transportType as keyof typeof transportMultipliers] || 1)
    const ticketBonus = ticketType ? transportBonus * (ticketMultipliers[ticketType as keyof typeof ticketMultipliers] || 1) : transportBonus
    ...
    toast.success(`🚌 Public transport trip recorded! +${totalPoints} points earned for choosing sustainable transport!`)
    setTransportType('')
    setDistance('')
    setDuration('')
    setRoute('')
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          {recentTrips.map((trip) => (
            <div key={trip.id} className="flex items-center justify-between p-3 bg-blue-900/20 rounded-lg border border-blue-500/20">
        ...
        {/* Record New Trip */}
        <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-gradient-to-r from-blue-900/20 to-cyan-900/20 rounded-lg border border-blue-500/20">
          <h4 className="font-semibold text-blue-400 flex items-center gap-2">
        ...
        <div className="p-4 bg-gradient-to-r from-blue-500/10 to-cyan-500/10 rounded-lg border border-blue-500/20">
          <p className="text-sm text-blue-300">
            💡 <strong>Commuter Champion:</strong> Regular public transport use earns consistent rewards! Monthly passes and student discounts provide bonus multipliers for sustainable commuting!
          </p>
        </div>
      </CardContent>
    </Card>
