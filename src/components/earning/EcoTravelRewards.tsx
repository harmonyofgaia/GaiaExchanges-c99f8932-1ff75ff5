...
export function EcoTravelRewards() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    toast.success(`✈️ Eco travel recorded! +${totalPoints} points earned for sustainable exploration!`)
    setTravelType('')
    setDestination('')
    setTransportation('')
    setDuration('')
  ...
  const getTransportIcon = (transport: string) => {
    switch (transport) {
      case 'train_bus': return <Train className="h-4 w-4" />
      case 'bicycle': return <Compass className="h-4 w-4" />
      case 'electric_vehicle': return <Car className="h-4 w-4" />
  ...
  return (
    <Card className="border-cyan-500/30 bg-gradient-to-br from-cyan-900/20 via-blue-900/20 to-teal-900/20">
      ...
      <CardContent className="space-y-6">
        ...
        {/* Plan New Eco Trip */}
        <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-gradient-to-r from-cyan-900/20 to-blue-900/20 rounded-lg border border-cyan-500/20">
          <h4 className="font-semibold text-cyan-400 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Select value={travelType} onValueChange={setTravelType}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="sustainable_resort">🌿 Sustainable Resort (180 pts)</SelectItem>
            ...
            <div>
              ...
              <Select value={transportation} onValueChange={setTransportation}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="electric_vehicle">⚡ Electric Vehicle (1.8x multiplier)</SelectItem>
                  <SelectItem value="public_transport">🚌 Public Transport (2.2x multiplier)</SelectItem>
                  <SelectItem value="carpool">👥 Carpool (1.7x multiplier)</SelectItem>
                  <SelectItem value="hybrid_vehicle">🔋 Hybrid Vehicle (1.5x multiplier)</SelectItem>
                  <SelectItem value="airplane_offset">✈️ Flight + Carbon Offset (1.3x multiplier)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium mb-2">Carbon Offset (kg CO2) - Optional</label>
              <Input
                type="number"
                step="0.1"
                value={carbonOffset}
        ...
        <div className="p-4 bg-gradient-to-r from-cyan-500/10 to-blue-500/10 rounded-lg border border-cyan-500/20">
          <p className="text-sm text-cyan-300">
            💡 <strong>Travel Smart:</strong> Sustainable travel earns massive rewards! Choose eco-friendly transportation and accommodations to maximize your GAiA earnings while protecting the planet!
          </p>
        </div>
      </CardContent>
    </Card>
