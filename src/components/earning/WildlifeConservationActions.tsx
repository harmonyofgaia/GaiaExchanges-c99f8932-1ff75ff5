...
export function WildlifeConservationActions() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const points = (basePoints[conservationType as keyof typeof basePoints] || 50) + (parseFloat(habitatArea) * 2)
    ...
    toast.success(`Wildlife conservation recorded! +${Math.floor(points)} points earned for protecting nature`)
    setConservationType('')
    setSpeciesHelped('')
    setHabitatArea('')
    setActivityDuration('')
      ...
      <CardContent>
          ...
          <div>
            ...
            <Select value={conservationType} onValueChange={setConservationType}>
              ...
              <SelectContent>
                ...
                <SelectItem value="nest_box_installation">Nest Box Installation (50 pts)</SelectItem>
                <SelectItem value="invasive_species_removal">Invasive Species Removal (80 pts)</SelectItem>
                <SelectItem value="wildlife_corridor_creation">Wildlife Corridor Creation (120 pts)</SelectItem>
        ...
        <div className="mt-4 p-3 bg-emerald-900/20 rounded-lg border border-emerald-500/30">
          <p className="text-sm text-emerald-300">
            💡 <strong>Biodiversity Bonus:</strong> Wildlife conservation activities get premium multipliers for protecting endangered species!
          </p>
        </div>
      </CardContent>
    </Card>
