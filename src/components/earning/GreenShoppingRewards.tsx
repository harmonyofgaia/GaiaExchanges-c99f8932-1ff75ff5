...
export function GreenShoppingRewards() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const categoryBonus = basePoints * (categoryMultipliers[itemCategory as keyof typeof categoryMultipliers] || 1)
    const shopBonus = categoryBonus * (shopMultipliers[shopType as keyof typeof shopMultipliers] || 1)
    
    let certificationBonus = 0
    certifications.forEach(cert => {
      certificationBonus += shopBonus * (certificationBonuses[cert as keyof typeof certificationBonuses] || 0)
    })
    
    const packagingBonus = packaging ? shopBonus * ((packagingBonuses[packaging as keyof typeof packagingBonuses] || 1) - 1) : 0
    ...
    toast.success(`🛍️ Green shopping recorded! +${totalPoints} points earned for sustainable choices!`)
    setShopType('')
    setItemCategory('')
    setAmount('')
    setCertifications([])
  ...
  const getCertificationIcon = (cert: string) => {
    switch (cert) {
      case 'organic': return '🌱'
      case 'fair_trade': return '⚖️'
      case 'plastic_free': return '🚫'
    ...
    <Card className="border-emerald-500/30 bg-gradient-to-br from-emerald-900/20 via-green-900/20 to-teal-900/20">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-emerald-400">💚 Your Sustainable Shopping Impact</h3>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div className="text-center p-4 bg-emerald-900/30 rounded-lg border border-emerald-500/20">
              <div className="text-xl font-bold text-emerald-400">{shoppingStats.ecoProductsBought}</div>
            ...
            <div className="text-center p-4 bg-green-900/30 rounded-lg border border-green-500/20">
              <div className="text-xl font-bold text-green-400">{shoppingStats.localBusinessesSupported}</div>
            ...
            <div className="text-center p-4 bg-blue-900/30 rounded-lg border border-blue-500/20">
              <div className="text-xl font-bold text-blue-400">{shoppingStats.plasticAvoided}kg</div>
            ...
            <div className="text-center p-4 bg-purple-900/30 rounded-lg border border-purple-500/20">
              <div className="text-xl font-bold text-purple-400">{shoppingStats.carbonFootprintReduced}kg</div>
        ...
        <div className="space-y-3">
          ...
          {recentPurchases.map((purchase) => (
            <div key={purchase.id} className="p-4 bg-emerald-900/20 rounded-lg border border-emerald-500/20">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      {purchase.certifications.map((cert, index) => (
                        <Badge key={index} variant="outline" className="text-xs border-emerald-500/30">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-gradient-to-r from-emerald-900/20 to-green-900/20 rounded-lg border border-emerald-500/20">
          <h4 className="font-semibold text-emerald-400 flex items-center gap-2">
          ...
          {/* Certifications */}
          <div>
            <label className="block text-sm font-medium mb-2">Product Certifications (Select all that apply)</label>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {[
                { value: 'organic', label: '🌱 Organic', bonus: '+30%' },
                { value: 'fair_trade', label: '⚖️ Fair Trade', bonus: '+40%' },
                { value: 'plastic_free', label: '🚫 Plastic Free', bonus: '+50%' },
                { value: 'carbon_neutral', label: '🌍 Carbon Neutral', bonus: '+60%' },
                { value: 'biodegradable', label: '🍃 Biodegradable', bonus: '+30%' },
        ...
        <div className="p-4 bg-gradient-to-r from-emerald-500/10 to-green-500/10 rounded-lg border border-emerald-500/20">
          <p className="text-sm text-emerald-300">
            💡 <strong>Conscious Consumer Bonus:</strong> Certifications and packaging choices multiply your rewards! Support local businesses and choose plastic-free options for maximum GAiA earnings!
          </p>
        </div>
      </CardContent>
    </Card>
