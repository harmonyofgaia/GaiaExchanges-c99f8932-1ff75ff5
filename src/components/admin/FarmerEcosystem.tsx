...
export function FarmerEcosystem() {
  ...
  const totalForestInvestment = forestProjects.reduce((sum, project) => sum + project.totalInvestment, 0)
        ...
        <Card className="bg-gradient-to-br from-green-900/20 to-emerald-900/20 border border-green-500/30">
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-2xl font-bold text-green-400">${totalEventRevenue.toLocaleString()}</div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="dashboard" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="bg-gradient-to-br from-green-900/20 to-emerald-900/20 border border-green-500/30">
              <CardHeader>
                <CardTitle className="text-lg text-green-400">Forest Development Progress</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                {forestProjects.map((project) => (
                  <div key={project.id} className="p-3 bg-gray-800/50 rounded">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      ...
                      <span>Layers: {project.layers}/{project.targetLayers}</span>
                      <span>${project.totalInvestment.toLocaleString()}</span>
                    </div>
                    <div className="mt-2 w-full bg-gray-700 rounded-full h-1.5">
                      <div 
                        className="bg-gradient-to-r from-green-500 to-emerald-500 h-1.5 rounded-full"
                        style={{ width: `${(project.layers / project.targetLayers) * 100}%` }}
                      ></div>
                    </div>
                  </div>
                ))}
          ...
          <Card className="bg-gradient-to-br from-yellow-900/20 to-orange-900/20 border border-yellow-500/30">
            <CardHeader>
              <CardTitle className="text-lg text-yellow-400">💰 Profit-to-Forest Investment Flow</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-400">${totalForestInvestment.toLocaleString()}</div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-sm text-muted-foreground">Layers Created (Target: 7 per forest)</div>
                </div>
              </div>
              <div className="mt-4 p-4 bg-green-900/20 border border-green-500/30 rounded-lg">
                <h4 className="text-green-400 font-semibold mb-2">🌱 Ecosystem Impact</h4>
                <p className="text-sm text-muted-foreground">
                  Every farmer event generates profits that directly fund the creation of 7-layered living forests. 
                  These forests provide sustainable food production, carbon sequestration, and biodiversity restoration.
                </p>
              </div>
            </CardContent>
          </Card>
                          ...
                          {event.profitGenerated > 0 && (
                            <div className="mt-2 p-2 bg-green-900/20 border border-green-500/30 rounded">
                              <span className="text-green-400 text-sm font-medium">
                                💰 Profit Generated: ${event.profitGenerated} → {event.forestLayers} Forest Layers Funded
                              </span>
                            </div>
                          )}
                        </div>
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3 text-sm">
                        <div>
                          ...
                          <div className="font-medium text-green-400">{project.layers} / {project.targetLayers}</div>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Investment:</span>
                          <div className="font-medium text-blue-400">${project.totalInvestment.toLocaleString()}</div>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Completion:</span>
                          <div className="font-medium text-purple-400">{project.completionDate}</div>
                        </div>
                        <div>
                      ...
                      {project.layers === project.targetLayers && (
                        <div className="mt-2 p-2 bg-green-900/30 border border-green-500/50 rounded">
                          <span className="text-green-400 text-sm font-medium">
                            🎉 Complete 7-Layer Ecosystem! Providing sustainable food, carbon capture & biodiversity.
                          </span>
                        </div>
                      )}
                    </CardContent>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <div className="font-medium text-purple-400">
                      {Math.floor((newEvent.ticketPrice * newEvent.expectedAttendees) / 5000)} layers
                    </div>
                  </div>
                </div>
              </div>
