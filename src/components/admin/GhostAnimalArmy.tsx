...
export function GhostAnimalArmy() {
  const [ghosts, setGhosts] = useState<GhostAvatar[]>([
    { id: 'ghost-1', name: 'Spirit Guardian', level: 47, energy: 95, assignedAnimal: 'Dragon Alpha', status: 'active', helpCount: 2847, position: { x: 20, y: 30 } },
    { id: 'ghost-2', name: 'Phantom Helper', level: 52, energy: 88, assignedAnimal: 'Koala AI', status: 'active', helpCount: 3192, position: { x: 60, y: 45 } },
    { id: 'ghost-3', name: 'Shadow Warrior', level: 39, energy: 100, assignedAnimal: 'Evolution Beast', status: 'upgrading', helpCount: 1956, position: { x: 40, y: 70 } },
    { id: 'ghost-4', name: 'Ethereal Scout', level: 61, energy: 76, assignedAnimal: 'Training Animal', status: 'active', helpCount: 4203, position: { x: 80, y: 25 } },
    { id: 'ghost-5', name: 'Mystic Supporter', level: 44, energy: 92, assignedAnimal: 'Defense Dragon', status: 'resting', helpCount: 2654, position: { x: 15, y: 60 } }
  ])
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'resting': return 'bg-blue-500/20 text-blue-400'
      case 'upgrading': return 'bg-purple-500/20 text-purple-400'
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="text-center p-4 bg-green-900/30 rounded-lg">
              ...
              <div className="text-2xl font-bold text-green-400">{armyStats.totalHelps.toLocaleString()}</div>
            ...
            {ghosts.map((ghost) => (
              <div
                ...
                className={`absolute w-4 h-4 rounded-full ${
                  ghost.status === 'active' ? 'bg-purple-400 animate-pulse' : 
                  ghost.status === 'resting' ? 'bg-blue-400' : 'bg-yellow-400'
      ...
      <Card className="bg-gradient-to-r from-indigo-900/50 to-purple-900/50 border-indigo-500/50">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {ghosts.map((ghost) => (
              <div key={ghost.id} className="bg-black/30 rounded-lg p-4 border border-purple-500/30">
