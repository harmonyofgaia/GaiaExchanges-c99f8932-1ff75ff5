...
export function WalletConnectionManager() {
  const [walletConnections, setWalletConnections] = useState<WalletConnection[]>([
    {
      ...
      wallet_address: GAIA_TOKEN.WALLET_ADDRESS,
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'community': return <DollarSign className="h-4 w-4" />
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'green_projects': return 'bg-green-600'
      case 'burning': return 'bg-orange-600'
      case 'community': return 'bg-blue-600'
      case 'humanity': return 'bg-pink-600'
  ...
  const updateAllocation = (id: string, percentage: number) => {
    ...
    setWalletConnections(prev =>
      prev.map(conn =>
        conn.id === id
          ? { ...conn, allocation_percentage: percentage }
          : conn
      )
    )
  }
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-3 rounded-lg bg-purple-900/30">
              <div className="text-2xl font-bold text-purple-400">{totalReceived.toLocaleString()}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {walletConnections.map((connection) => (
          <Card key={connection.id} className="border-green-500/20 bg-gradient-to-br from-green-900/10 to-emerald-900/10">
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                  ...
                  <div>
                    {editingId === connection.id ? (
                      <Input
                        ...
                        onChange={(e) => setEditData(prev => ({ ...prev, project_name: e.target.value }))}
                        className="font-bold text-sm"
                      />
                    ) : (
                      <h3 className="font-bold text-sm text-green-400">{connection.project_name}</h3>
                    )}
                    {editingId === connection.id ? (
                      <Input
                ...
                <div className="flex items-center gap-2">
                  <Badge className={connection.status === 'active' ? 'bg-green-600' : 'bg-gray-600'}>
              ...
              <div className="space-y-3">
                  ...
                  {editingId === connection.id ? (
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        onChange={(e) => setEditData(prev => ({ ...prev, allocation_percentage: parseInt(e.target.value) || 0 }))}
                        className="w-16 text-xs"
                      />
                      <span className="text-xs">%</span>
                    </div>
                  ) : (
                    <span className="font-bold text-green-400">{connection.allocation_percentage}%</span>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold">{connection.total_received.toLocaleString()} GAIA</span>
                </div>
                ...
                <div className="bg-black/30 p-3 rounded">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    >
                      {showAddresses[connection.id] ? <EyeOff className="h-3 w-3" /> : <Eye className="h-3 w-3" />}
                    </Button>
                  </div>
                  {editingId === connection.id ? (
                    <Input
                      value={editData.wallet_address || ''}
                      onChange={(e) => setEditData(prev => ({ ...prev, wallet_address: e.target.value }))}
