...
export function ThunderstormDefense() {
    ...
    const interval = setInterval(() => {
      ...
      if (Math.random() < 0.3) {
        const newThreat: ThreatAlert = {
          ...
          type: ['intrusion', 'ddos', 'malware', 'data_breach'][Math.floor(Math.random() * 4)] as any,
        ...
        if (newThreat.blocked) {
          setMetrics(prev => ({
            ...
            intrustionAttempts: newThreat.type === 'intrusion' ? prev.intrustionAttempts + 1 : prev.intrustionAttempts,
            ddosAttacks: newThreat.type === 'ddos' ? prev.ddosAttacks + 1 : prev.ddosAttacks,
            malwareDetected: newThreat.type === 'malware' ? prev.malwareDetected + 1 : prev.malwareDetected,
            dataBreachPrevented: newThreat.type === 'data_breach' ? prev.dataBreachPrevented + 1 : prev.dataBreachPrevented
          }))
        }
      }
    }, 3000)
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'malware': return <Cpu className="h-4 w-4" />
      ...
      <Card className="border-blue-500/30 bg-gradient-to-r from-blue-900/30 to-purple-900/30">
        <CardHeader>
          <CardTitle className="text-blue-400 flex items-center gap-2">
            ...
            ⚡ Thunderstorm Defense Mechanism
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <Card className="bg-blue-900/20 border-blue-500/30">
              <CardContent className="pt-4 text-center">
                <div className="text-2xl font-bold text-blue-400">{metrics.intrustionAttempts.toLocaleString()}</div>
            ...
            <Card className="bg-orange-900/20 border-orange-500/30">
              <CardContent className="pt-4 text-center">
                <div className="text-2xl font-bold text-orange-400">{metrics.ddosAttacks.toLocaleString()}</div>
            ...
            <Card className="bg-purple-900/20 border-purple-500/30">
              <CardContent className="pt-4 text-center">
                <div className="text-2xl font-bold text-purple-400">{metrics.malwareDetected.toLocaleString()}</div>
            ...
            <Card className="bg-red-900/20 border-red-500/30">
              <CardContent className="pt-4 text-center">
                <div className="text-2xl font-bold text-red-400">{metrics.dataBreachPrevented.toLocaleString()}</div>
            ...
            <CardContent className="pt-4">
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {threats.length === 0 ? (
                  ...
                  threats.map(threat => (
                    <div key={threat.id} className="flex items-center justify-between p-3 bg-black/40 rounded border border-border/20">
                      <div className="flex items-center gap-3">
                        {getTypeIcon(threat.type)}
                        <div>
                          <div className="font-semibold text-white capitalize">{threat.type.replace('_', ' ')}</div>
              ...
              <CardContent className="pt-4">
                <div className="text-center">
                  ...
                  <p className="text-yellow-300">
                    Maximum security protocols engaged. All threats will be eliminated with extreme prejudice.
                  </p>
