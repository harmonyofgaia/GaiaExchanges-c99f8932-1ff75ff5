...
interface PrivacyEvent {
  id: string
  type: 'data_access' | 'login_attempt' | 'data_export' | 'permission_change' | 'gdpr_request' | 'breach_attempt'
  user_id: string
  username: string
  description: string
...
export function PrivacyWatchdog() {
  ...
  const resolvePrivacyEvent = (eventId: string) => {
    setPrivacyEvents(prev => prev.map(event => 
      event.id === eventId 
    ...
    setGDPRRequests(prev => prev.map(request => 
      request.id === requestId 
        ? { 
            ...
            processing_notes: action === 'approve' ? 'Request processed successfully' : 'Request rejected due to insufficient verification'
          }
        : request
    ))
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'investigating': return <Search className="h-4 w-4 text-blue-400" />
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'resolved': return 'border-green-500/50 text-green-400'
      case 'blocked': return 'border-red-500/50 text-red-400'
      case 'investigating': return 'border-blue-500/50 text-blue-400'
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'breach_attempt': return <Shield className="h-4 w-4" />
      default: return <Eye className="h-4 w-4" />
    }
  }

  const filteredEvents = privacyEvents.filter(event => 
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-400">{privacyMetrics.total_users_protected.toLocaleString()}</div>
              <div className="text-sm text-muted-foreground">Users Protected</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-green-400">{privacyMetrics.privacy_events_today}</div>
              <div className="text-sm text-muted-foreground">Events Today</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-400">{privacyMetrics.data_requests_processed}</div>
              <div className="text-sm text-muted-foreground">Requests Processed</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-yellow-400">{privacyMetrics.gdpr_compliance_score.toFixed(1)}%</div>
              <div className="text-sm text-muted-foreground">GDPR Compliance</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-red-400">{privacyMetrics.unauthorized_access_blocked}</div>
              <div className="text-sm text-muted-foreground">Threats Blocked</div>
            </div>
            <div className="text-center">
              <Badge variant="outline" className={privacyMetrics.data_retention_active ? 'border-green-500/50 text-green-400' : 'border-red-500/50 text-red-400'}>
            ...
            <div className="text-center">
              <Badge variant="outline" className={privacyMetrics.audit_trail_complete ? 'border-green-500/50 text-green-400' : 'border-yellow-500/50 text-yellow-400'}>
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <Button
              ...
              className={isMonitoring ? 'border-red-500/50 text-red-400' : 'border-green-500/50 text-green-400'}
            >
              {isMonitoring ? (
                <>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3 text-sm">
                        ...
                        <div>
                          ...
                          <div className="font-medium text-yellow-400">{event.timestamp.toLocaleString()}</div>
                        </div>
                      </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-3 text-sm">
                        ...
                        <div>
                          ...
                          <div className="font-medium text-blue-400">{request.submission_date.toLocaleDateString()}</div>
                        </div>
                        {request.completion_date && (
                          <div>
                            <div className="text-muted-foreground">Completed</div>
                            <div className="font-medium text-green-400">{request.completion_date.toLocaleDateString()}</div>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <h3 className="font-semibold text-white">{access.data_type.replace('_', ' ')}</h3>
                        <Badge variant="outline" className="border-green-500/50 text-green-400">
                          {access.access_level}
                        </Badge>
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3 text-sm">
                        ...
                        <div>
                          ...
                          <div className="font-medium text-blue-400">{access.purpose.replace('_', ' ')}</div>
                        </div>
                        <div>
                          <div className="text-muted-foreground">Authorized By</div>
                          <div className="font-medium text-purple-400">{access.authorized_by.replace('_', ' ')}</div>
                        </div>
                        <div>
                          <div className="text-muted-foreground">Retention</div>
                          <div className="font-medium text-yellow-400">{access.retention_period.replace('_', ' ')}</div>
                        </div>
                      </div>
