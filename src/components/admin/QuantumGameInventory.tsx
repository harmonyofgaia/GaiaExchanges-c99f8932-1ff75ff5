            ...
            <TabsContent value="assets" className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {gameAssets.map((asset) => (
                  <Card key={asset.id} className={`bg-gradient-to-br ${getRarityColor(asset.rarity)} bg-opacity-10 border-2`}>
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-xs">
                          ...
                          <span className="text-green-400">{asset.powerLevel.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between text-xs">
            ...
            <TabsContent value="levels" className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                {gameLevels.slice(0, 20).map((level) => (
                  <Card key={level.id} className={`bg-black/40 border ${
                    level.type === 'surface' ? 'border-green-500/30' :
                    level.type === 'underwater' ? 'border-blue-500/30' : 'border-purple-500/30'
                  }`}>
                    <CardContent className="p-3">
                      ...
                      <Badge className={`text-xs ${level.unlocked ? 'bg-green-600' : 'bg-gray-600'}`}>
            ...
            <TabsContent value="biomech" className="space-y-4">
              <div className="text-center p-8 bg-gradient-to-r from-red-900/20 to-purple-900/20 rounded-lg border border-red-500/30">
                ...
                <p className="text-muted-foreground mb-4">
                  Quantum-powered biomechanical weapon creation system.
                  Fusion of organic and digital elements for ultimate power.
                </p>
