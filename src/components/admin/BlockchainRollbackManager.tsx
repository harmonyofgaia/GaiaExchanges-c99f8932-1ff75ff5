      ...
      <Card className="bg-gradient-to-r from-blue-900/40 to-purple-900/40 border-blue-500/50">
        <CardHeader>
          ...
          <div className="flex gap-4">
            ...
            <Badge className={`${rollbackInProgress ? 'bg-yellow-600 animate-pulse' : 'bg-blue-600'}`}>
            ...
            <Card className="bg-yellow-900/20 border-yellow-500/30">
              <CardContent className="p-4">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <Badge className={`
                    ${activeRollback.status === 'completed' ? 'bg-green-600' : 
                      activeRollback.status === 'processing' ? 'bg-yellow-600 animate-pulse' : 'bg-gray-600'}
                  `}>
                ...
                <div className="flex justify-between text-sm text-muted-foreground">
                  ...
                  <span>ETA: {Math.max(0, activeRollback.estimatedTime - (activeRollback.progress * activeRollback.estimatedTime / 100)).toFixed(0)}s</span>
                </div>
              </CardContent>
            </Card>
          )}
            ...
            {blockchainStates.map((state) => (
              <div key={state.id} className="p-4 bg-black/30 rounded-lg border border-gray-600">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <div className="font-bold text-white">Block {state.blockHeight.toLocaleString()}</div>
                    <div className="text-sm text-muted-foreground">{state.description}</div>
                  </div>
                  <div className="flex gap-2">
                    <Badge className={`${getValidationColor(state.validation)} bg-transparent border`}>
