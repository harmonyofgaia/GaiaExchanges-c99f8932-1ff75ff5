...
export function RuleSystemManagement() {
    ...
    const mockRules: Rule[] = [
      {
        ...
        conditions: [
          ...
          { field: 'content_similarity', operator: 'greater_than', value: '85' }
        ],
      ...
      },
      {
        ...
        enforcement: 'warning',
        conditions: [
          { field: 'content_type', operator: 'contains', value: 'inappropriate' }
        ],
        assignedTo: [{ type: 'all', targetName: 'All Users' }],
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'content': return AlertTriangle
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-green-400">{rules.filter(r => r.isActive).length}</div>
              <div className="text-xs text-muted-foreground">Active Rules</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-red-400">{rules.reduce((sum, r) => sum + r.violations, 0)}</div>
              <div className="text-xs text-muted-foreground">Total Violations</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-400">{rules.filter(r => r.priority === 'critical').length}</div>
              <div className="text-xs text-muted-foreground">Critical Rules</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-yellow-400">{rules.filter(r => r.autoEnforcement).length}</div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <div className="grid gap-4">
            {filteredRules.map((rule) => {
              ...
              return (
                <Card key={rule.id} className="border border-gray-700 hover:border-purple-500/50 transition-colors">
                  <CardHeader>
        ...
        <TabsContent value="create" className="space-y-4">
          <RuleCreator rule={selectedRule} onSave={(rule) => {
            if (selectedRule) {
            ...
            <CardContent>
              <div className="space-y-4">
                {rules.filter(r => r.isActive).map((rule) => (
                  <div key={rule.id} className="flex items-center justify-between p-4 border border-gray-700 rounded-lg">
                    <div className="flex items-center gap-3">
                ...
                <div className="flex items-center justify-between p-3 border border-red-500/30 rounded-lg bg-red-900/10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">User exceeded message limit</div>
                    </div>
                  </div>
                  <div className="text-right">
                ...
                <div className="flex items-center justify-between p-3 border border-yellow-500/30 rounded-lg bg-yellow-900/10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">Inappropriate content detected</div>
                    </div>
                  </div>
                  <div className="text-right">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(value: string) => setFormData({ ...formData, enforcement: value })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
