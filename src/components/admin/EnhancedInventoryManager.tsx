...
export function EnhancedInventoryManager() {
  ...
  const loadEnhancedInventory = useCallback(() => {
    const items: InventoryItem[] = [
      // Mythical Weapons
      {
        ...
        powerLevel: 99999,
        description:
          "Legendary sword forged from quantum dragon scales with reality-cutting abilities",
        fileSize: "15.7 GB",
        value: 1000000,
        quantity: 1,
      },
  ...
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      case "mythical":
        return "from-red-600 via-purple-600 to-pink-600";
      case "quantum":
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-center">
          ...
          <div className="p-2 bg-red-900/30 rounded">
            <div className="text-lg font-bold text-red-400">
              {inventory.filter((i) => i.rarity === "mythical").length}
            </div>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredItems.map((item) => (
            <Card
              key={item.id}
              className={`bg-gradient-to-br ${getRarityColor(item.rarity)} bg-opacity-10 border-2 hover:scale-105 transition-all`}
            >
              <CardContent className="p-4">
                ...
                <Button
                  className={`w-full mt-3 bg-gradient-to-r ${getRarityColor(item.rarity)} hover:opacity-90 text-xs`}
                >
