...
export function PlatformManagement() {
  const [exchanges, setExchanges] = useState<ExchangeIntegration[]>([
    { name: 'Binance', status: 'active', volume24h: 2400000000, fees: 0.1, pairs: 847, uptime: 99.9 },
    { name: 'Coinbase Pro', status: 'active', volume24h: 1200000000, fees: 0.5, pairs: 289, uptime: 99.8 },
    { name: 'KuCoin', status: 'pending', volume24h: 800000000, fees: 0.1, pairs: 456, uptime: 99.7 },
    { name: 'Huobi', status: 'maintenance', volume24h: 600000000, fees: 0.2, pairs: 234, uptime: 98.5 }
  ])

  const [investments, setInvestments] = useState<InvestmentOpportunity[]>([
    { title: 'European Market Expansion', type: 'expansion', amount: '$25M', roi: '340%', status: 'open', priority: 'high' },
    { title: 'Strategic Partnership with Revolut', type: 'partnership', amount: '$15M', roi: '280%', status: 'negotiating', priority: 'high' },
    { title: 'Asian Infrastructure Investment', type: 'investment', amount: '$50M', roi: '450%', status: 'open', priority: 'medium' },
    { title: 'DeFi Protocol Integration', type: 'partnership', amount: '$8M', roi: '220%', status: 'completed', priority: 'low' }
  ])
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'negotiating': return 'bg-orange-600'
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-400">${(metrics.totalVolume / 1e9).toFixed(1)}B</p>
              </div>
                  ...
                  <div key={index} className="p-4 rounded-lg bg-black/30 border border-gray-500/30">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <div className="font-bold text-purple-400">{exchange.uptime.toFixed(1)}%</div>
                      </div>
                    </div>
                    ...
                    <div className="mt-3">
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="text-green-400">
                          {exchange.status === 'active' ? '100%' : exchange.status === 'pending' ? '75%' : '45%'}
                        </span>
                      </div>
                      <Progress 
                        value={exchange.status === 'active' ? 100 : exchange.status === 'pending' ? 75 : 45} 
                        className="h-2"
                      />
                    </div>
                  </div>
                  ...
                  <div key={index} className="p-4 rounded-lg bg-black/30 border border-gray-500/30">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        ...
                        <Badge variant="outline" className={`${getPriorityColor(investment.priority)} border-current text-xs`}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <div className="font-bold text-purple-400 capitalize">{investment.type}</div>
                      </div>
                    </div>
                  </div>
                ))}
