...
export function RealTimeAdminMetrics() {
  ...
  const updateMetricsRealTime = async () => {
    try {
      ...
      const updates = [
        { name: 'active_users', increment: Math.floor(Math.random() * 50) - 25 },
        { name: 'total_transactions', increment: Math.floor(Math.random() * 100) + 10 },
        { name: 'system_load', value: Math.max(0, Math.min(100, 20 + (Math.random() - 0.5) * 10)) },
        { name: 'growth_rate', value: Math.max(0, 150 + (Math.random() - 0.5) * 20) },
        { name: 'community_score', value: Math.max(0, Math.min(100, 94 + (Math.random() - 0.5) * 5)) }
      ]
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {Object.entries(metrics).map(([name, value]) => {
            const metricType = name.includes('uptime') || name.includes('score') || name.includes('load') || name.includes('growth') ? 'percentage' : 'counter'
            return (
