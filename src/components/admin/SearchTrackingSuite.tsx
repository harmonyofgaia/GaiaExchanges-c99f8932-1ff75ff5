...
export function SearchTrackingSuite() {
  ...
  useEffect(() => {
    const updateStats = () => {
      ...
      if (Math.random() < 0.3) {
        setTrackingTargets(prev => prev.map(target => {
          if (Math.random() < 0.2) {
            return {
              ...target,
              lastSeen: new Date(),
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'analyzing': return 'bg-purple-600'
      case 'tracking': return 'bg-yellow-600'
      case 'located': return 'bg-orange-600'
      case 'eliminated': return 'bg-red-600'
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="text-center p-4 bg-green-900/30 rounded-lg">
              ...
              <div className="text-2xl font-bold text-green-400">{totalResults.toLocaleString()}</div>
            ...
            {searchEngines.map((engine) => (
              <Card key={engine.id} className="border-cyan-500/30 bg-cyan-900/20">
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm">
                    <span>Results:</span>
                    <span className="text-purple-400 font-bold">{engine.resultsFound.toLocaleString()}</span>
                  </div>
                </CardContent>
              </Card>
            ))}
