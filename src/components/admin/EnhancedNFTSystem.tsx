    ...
    const enhancedNFTCollection: EnhancedNFT[] = [
      {
        ...
        rarity: 'Divine',
        sprite: '🔥',
        stats: { attackPower: 100, defensePower: 70, speed: 85, energy: 90, durability: 80, specialAbility: 95 },
        abilities: {
      ...
      },
      {
        ...
        rarity: 'Legendary',
        sprite: '🛡️',
        stats: { attackPower: 60, defensePower: 100, speed: 40, energy: 85, durability: 95, specialAbility: 80 },
        abilities: {
          primary: 'Stone Wall',
      ...
      },
      {
        ...
        rarity: 'Mythical',
        sprite: '⚡',
        stats: { attackPower: 90, defensePower: 50, speed: 100, energy: 95, durability: 70, specialAbility: 100 },
        abilities: {
      ...
      },
      {
        ...
        rarity: 'Epic',
        sprite: '🌊',
        stats: { attackPower: 70, defensePower: 80, speed: 75, energy: 100, durability: 90, specialAbility: 85 },
        abilities: {
      ...
      },
      {
        ...
        rarity: 'Divine',
        sprite: '🌑',
        stats: { attackPower: 95, defensePower: 85, speed: 90, energy: 80, durability: 75, specialAbility: 100 },
        abilities: {
...
-------------------
  ...
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      case 'Divine': return 'from-purple-600 via-pink-600 to-gold-600'
      case 'Mythical': return 'from-purple-600 to-pink-600'
      case 'Legendary': return 'from-gold-600 to-orange-600'
      ...
      <Card className="border-2 border-purple-500/50 bg-gradient-to-br from-purple-900/30 to-pink-900/30">
        <CardHeader>
          ...
          <p className="text-muted-foreground">
            Advanced NFT system with elemental properties, comprehensive documentation, and admin reordering
          </p>
        </CardHeader>
        <CardContent>
          ...
          >
            <CardContent className="p-4 space-y-4">
              ...
              <div className="text-center space-y-1">
                <div className={`text-sm font-bold ${getElementColor(nft.element)}`}>{nft.type}</div>
                <div className="text-xs text-muted-foreground">{nft.descriptions.lore.substring(0, 80)}...</div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div><span className="text-yellow-400">Primary:</span> {selectedNFT.abilities.primary}</div>
                    <div><span className="text-orange-400">Secondary:</span> {selectedNFT.abilities.secondary}</div>
                    <div><span className="text-red-400">Ultimate:</span> {selectedNFT.abilities.ultimate}</div>
                  </div>
                </div>
                
                <div>
                  <h4 className="text-lg font-bold text-white mb-2">Combat Style</h4>
                  <p className="text-sm text-muted-foreground">{selectedNFT.descriptions.combatStyle}</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div><span className="text-green-400">Resistant:</span> {selectedNFT.elementalProperties.resistant.join(', ')}</div>
                    <div><span className="text-red-400">Vulnerable:</span> {selectedNFT.elementalProperties.vulnerable.join(', ')}</div>
                    <div><span className="text-yellow-400">Super Effective:</span> {selectedNFT.elementalProperties.superEffective.join(', ')}</div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div><span className="text-green-400">Strengths:</span> {selectedNFT.descriptions.strengths.join(', ')}</div>
                    <div><span className="text-red-400">Weaknesses:</span> {selectedNFT.descriptions.weaknesses.join(', ')}</div>
                  </div>
                </div>
              </div>
            </div>
