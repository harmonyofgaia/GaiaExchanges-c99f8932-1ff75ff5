...
export function PredictiveOptimization() {
    ...
    const interval = setInterval(() => {
      ...
      if (Math.random() > 0.7) {
        ...
        const newInsight: PredictiveInsight = {
          ...
          type: ['optimization', 'security', 'performance', 'user_experience'][Math.floor(Math.random() * 4)] as any,
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'user_experience': return <Target className="h-4 w-4" />
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'predicted': return 'bg-purple-600'
      default: return 'bg-gray-600'
    }
  }
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center justify-between p-4 bg-green-900/30 border border-green-500/30 rounded-lg">
          ...
          <Button
            ...
            className={autonomousMode ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}
          >
