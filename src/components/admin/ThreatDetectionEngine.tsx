...
export function ThreatDetectionEngine() {
  ...
  useEffect(() => {
    const initialMetrics: SystemMetric[] = [
      { id: '1', name: 'Server CPU Usage', value: 25, threshold: 80, status: 'healthy', icon: Cpu, unit: '%', lastChecked: Date.now() },
      { id: '2', name: 'Memory Usage', value: 67, threshold: 85, status: 'healthy', icon: HardDrive, unit: '%', lastChecked: Date.now() },
      { id: '3', name: 'Database Performance', value: 92, threshold: 70, status: 'healthy', icon: Database, unit: '%', lastChecked: Date.now() },
      { id: '4', name: 'Network Latency', value: 45, threshold: 200, status: 'healthy', icon: Network, unit: 'ms', lastChecked: Date.now() },
      { id: '5', name: 'API Response Time', value: 125, threshold: 500, status: 'healthy', icon: Globe, unit: 'ms', lastChecked: Date.now() },
      { id: '6', name: 'Active Sessions', value: 1247, threshold: 5000, status: 'healthy', icon: Users, unit: '', lastChecked: Date.now() },
      { id: '7', name: 'Failed Login Attempts', value: 3, threshold: 50, status: 'healthy', icon: Lock, unit: '/hour', lastChecked: Date.now() },
      { id: '8', name: 'File System Integrity', value: 100, threshold: 95, status: 'healthy', icon: HardDrive, unit: '%', lastChecked: Date.now() },
      { id: '9', name: 'Network Traffic', value: 234, threshold: 10000, status: 'healthy', icon: Wifi, unit: 'MB/s', lastChecked: Date.now() },
      { id: '10', name: 'Error Rate', value: 0.02, threshold: 1, status: 'healthy', icon: AlertTriangle, unit: '%', lastChecked: Date.now() }
    ]
  ...
  useEffect(() => {
    ...
    const monitoringInterval = setInterval(() => {
      ...
      setSystemMetrics(prev => prev.map(metric => {
        ...
        if (metric.name.includes('Error') || metric.name.includes('Failed') || metric.name.includes('Latency') || metric.name.includes('Response')) {
      ...
      setSystemMetrics(current => {
        ...
        if (criticalMetrics.length > 0 || Math.random() < 0.03) {
          ...
          const newThreat: ThreatIndicator = {
            ...
            severity: criticalMetrics.length > 0 ? Math.floor(Math.random() * 3) + 8 : Math.floor(Math.random() * 4) + 4,
            description: criticalMetrics.length > 0 
              ? `Critical system metric detected: ${criticalMetrics[0].name}` 
              : 'Anomalous behavior pattern detected',
            detected: Date.now(),
            source: ['Network Monitor', 'Database Scanner', 'File System Watcher', 'API Gateway', 'Security Scanner'][Math.floor(Math.random() * 5)],
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              ...
              <CheckCircle className={`h-8 w-8 ${overallHealth > 95 ? 'text-green-500' : overallHealth > 85 ? 'text-yellow-500' : 'text-red-500'}`} />
            </div>
        ...
        <CardHeader>
          <CardTitle className="flex items-center space-x-2">
            ...
            <div className="flex space-x-2 ml-auto">
              ...
              {criticalMetrics > 0 && <Badge variant="destructive">{criticalMetrics} Critical</Badge>}
            </div>
          </CardTitle>
        </CardHeader>
        <CardContent>
