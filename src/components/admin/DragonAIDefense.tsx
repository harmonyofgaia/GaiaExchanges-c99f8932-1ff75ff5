      ...
      <Card className="border-red-500/50 bg-gradient-to-r from-red-900/30 to-orange-900/30">
        <CardHeader>
          <CardTitle className="text-red-400 text-center text-3xl flex items-center justify-center gap-3">
            ...
            <Badge className={`${isActive ? 'bg-green-600 animate-pulse' : 'bg-red-600'} text-white`}>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-red-900/20 rounded-lg">
              <div className="text-3xl font-bold text-red-400">Level {dragonLevel.toLocaleString()}</div>
              <div className="text-sm text-muted-foreground">Dragon Evolution</div>
            </div>
            <div className="text-center p-4 bg-orange-900/20 rounded-lg">
              <div className="text-3xl font-bold text-orange-400">{(dragonPower / 1000000).toFixed(1)}M</div>
            ...
            <div className="text-center p-4 bg-purple-900/20 rounded-lg">
              <div className="text-3xl font-bold text-purple-400">{threatsBlocked.toLocaleString()}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="border-blue-500/30">
              ...
              <CardContent className="space-y-4">
                ...
                >
                  {isActive ? <Shield className="h-5 w-5 mr-2" /> : <Zap className="h-5 w-5 mr-2" />}
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <Button
                      ...
                      size="sm"
                    >
                      {notifications ? <Eye className="h-4 w-4" /> : <LockIcon className="h-4 w-4" />}
                    </Button>
                  </div>
                </div>
              </CardContent>
            ...
            <CardContent>
              <div className="space-y-4">
                {[
                  ...
                  { type: 'DDoS Attack', severity: 'Critical', status: 'Mitigated', time: '5 min ago' },
                  { type: 'XSS Attempt', severity: 'Medium', status: 'Blocked', time: '8 min ago' },
                  { type: 'Brute Force', severity: 'High', status: 'Blocked', time: '12 min ago' }
                ].map((threat, idx) => (
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="text-center p-4 bg-blue-900/20 rounded-lg">
                  ...
                  <div className="text-2xl font-bold text-blue-400">{Math.floor(Math.random() * 100) + 50}</div>
                  <div className="text-sm text-muted-foreground">AI Learning Rate</div>
                </div>
                <div className="text-center p-4 bg-red-900/20 rounded-lg">
                  <Flame className="h-8 w-8 text-red-400 mx-auto mb-2" />
                  <div className="text-2xl font-bold text-red-400">{Math.floor(Math.random() * 1000) + 500}</div>
          ...
          <Card className="border-gray-500/30">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button variant="default" size="sm">Automatic</Button>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
