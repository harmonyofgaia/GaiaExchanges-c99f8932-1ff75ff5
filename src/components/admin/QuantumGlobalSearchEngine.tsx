...
export function QuantumGlobalSearchEngine() {
  ...
  const getResultIcon = (type: string) => {
    switch (type) {
      case 'encrypted_file': return <Lock className="h-4 w-4" />
      case 'hacker_trace': return <Shield className="h-4 w-4" />
      ...
      <Card className="bg-gradient-to-r from-black to-purple-900/40 border-purple-500/50">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-purple-400">
            ...
            ⚡ QUANTUM GLOBAL SEARCH ENGINE - 20 QUANTUM COMPUTERS
            {quantumActive && <Badge className="bg-green-600 animate-pulse">QUANTUM ACTIVE</Badge>}
          </CardTitle>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-sm">
            <div className="text-center">
              <div className="text-lg font-bold text-green-400">{metrics.totalDocuments.toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Documents</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-blue-400">{metrics.encryptedFilesAccessed.toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Encrypted Files</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-purple-400">{metrics.databasesScanned.toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Databases</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-red-400">{metrics.hackerTracesFound.toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Hacker Traces</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-cyan-400">{metrics.globalReachPercentage.toFixed(1)}%</div>
