...
export function CustomAutomationBuilder() {
  ...
  const availableStepTypes = [
    {
      ...
      options: ['User Registration', 'Token Discovery', 'Security Alert', 'Mission Completion', 'Admin Login']
    },
    {
      type: 'condition',
    ...
    },
    {
      ...
      options: ['Send Notification', 'Update Database', 'Generate Report', 'Award Tokens', 'Block User', 'Deploy AI Defense']
    },
    {
      type: 'delay',
      name: 'Wait/Delay',
        ...
        steps: [
          {
            ...
            config: { threshold: 'medium', sources: ['network', 'api', 'user_behavior'] },
          ...
          },
          {
            ...
            config: { animals: ['bear', 'eagle', 'wolf'], deployment_strategy: 'adaptive' },
      ...
      },
      {
        ...
        steps: [
          ...
          },
          {
            ...
            config: { min_eco_score: 85, required_attributes: ['carbon_neutral', 'renewable_energy'] },
            position: { x: 300, y: 100 },
            connections: ['step-3']
          },
          {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'inactive': return 'border-gray-500/50 text-gray-400'
      case 'error': return 'border-red-500/50 text-red-400'
      case 'testing': return 'border-blue-500/50 text-blue-400'
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'token_management': return <Database className="h-4 w-4" />
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'security': return 'border-red-500/50 text-red-400'
      case 'token_management': return 'border-yellow-500/50 text-yellow-400'
      case 'user_engagement': return 'border-blue-500/50 text-blue-400'
      case 'environmental': return 'border-green-500/50 text-green-400'
      case 'analytics': return 'border-purple-500/50 text-purple-400'
      default: return 'border-gray-500/50 text-gray-400'
    }
  }
      ...
      <Card className="border-green-500/20 bg-gradient-to-r from-green-900/20 to-blue-900/20">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-400">{metrics.total_executions_today}</div>
              <div className="text-sm text-muted-foreground">Executions Today</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-yellow-400">{metrics.success_rate.toFixed(1)}%</div>
              <div className="text-sm text-muted-foreground">Success Rate</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-cyan-400">{metrics.time_saved_hours.toFixed(1)}h</div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
                        ...
                        <div>
                          <div className="text-sm text-muted-foreground">Success Rate</div>
                          <div className="font-bold text-green-400">{automation.success_rate.toFixed(1)}%</div>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <div className="font-bold text-yellow-400">{automation.last_run.toLocaleTimeString()}</div>
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col gap-2">
                      <Button
                        ...
                        className={automation.status === 'active' ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}
                      >
                        {automation.status === 'active' ? <Pause className="h-3 w-3 mr-1" /> : <Play className="h-3 w-3 mr-1" />}
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => { setSelectedAutomation(automation); setIsEditing(true) }}
                      >
            ...
            <Card className="border-blue-500/20">
              ...
              <CardContent>
                ...
                <div className="mt-6 space-y-4">
                  <h4 className="font-semibold text-white">Workflow Steps ({selectedAutomation.steps.length})</h4>
                  {selectedAutomation.steps.length > 0 ? (
                    <div className="space-y-3">
                      {selectedAutomation.steps.map((step, index) => (
                        <div key={step.id} className="flex items-center gap-3 p-3 border border-gray-500/20 rounded-lg">
            ...
            <Card className="border-gray-500/20">
              <CardContent className="pt-6 text-center">
                ...
                <p className="text-muted-foreground mb-4">
                  Select an automation from the list or create a new one to start building your workflow.
                </p>
          ...
          <div className="grid gap-4 md:grid-cols-2">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-green-400">{metrics.success_rate.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between items-center">
            ...
            <Card className="border-blue-500/20">
              ...
              <CardContent>
                <div className="space-y-3">
                  {[
                    { time: '2 min ago', event: 'AI Defense deployed', status: 'success' },
                    { time: '5 min ago', event: 'Token auto-approved', status: 'success' },
                    { time: '12 min ago', event: 'User engagement check', status: 'success' },
                    { time: '18 min ago', event: 'Security scan completed', status: 'success' },
                    { time: '25 min ago', event: 'Report generation failed', status: 'error' }
                  ].map((activity, index) => (
                    <div key={index} className="flex items-center justify-between p-2 border border-gray-500/20 rounded">
                      <div>
                        <div className="text-sm text-white">{activity.event}</div>
                        <div className="text-xs text-muted-foreground">{activity.time}</div>
                      </div>
                      <Badge variant="outline" className={activity.status === 'success' ? 'border-green-500/50 text-green-400' : 'border-red-500/50 text-red-400'}>
                        {activity.status}
                      </Badge>
                    </div>
                  ))}
