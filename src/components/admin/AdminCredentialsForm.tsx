...
export function AdminCredentialsForm({ loginType, onLoginSuccess }: AdminCredentialsFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const validCredentials = loginType === 'system' 
      ...
      if (credentials.username === validCredentials.username && 
        ...
        const successMessage = loginType === 'system' 
          ? '👑 SUPREME ADMIN ACCESS GRANTED!'
          : '🏦 VAULT ADMIN ACCESS GRANTED!'
        
        const description = loginType === 'system'
        ...
        const errorMessage = loginType === 'system' 
  ...
  return (
    <div className="space-y-4">
      <div className="text-center mb-4">
        ...
        <p className={`text-sm ${isSystem ? 'text-blue-300' : 'text-emerald-300'}`}>
          {subtitle}
        </p>
      </div>
