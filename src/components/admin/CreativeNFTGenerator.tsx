...
export function CreativeNFTGenerator() {
  ...
  const loadCreativeNFTs = () => {
    const creativeNFTs: CreativeNFT[] = [
      // Mythical Characters
      {
        ...
        createdBy: 'ai',
        stats: { attack: 95, defense: 90, magic: 100, speed: 85, luck: 90, charm: 95 },
      ...
      },
      {
        ...
        createdBy: 'ai',
        stats: { attack: 70, defense: 80, magic: 100, speed: 95, luck: 85, charm: 90 },
      ...
      },
      {
        ...
        createdBy: 'ai',
        stats: { attack: 80, defense: 85, magic: 90, speed: 90, luck: 95, charm: 100 },
  ...
  const generateNewNFT = async () => {
    ...
    const themes = ['Celestial', 'Mystical', 'Quantum', 'Rainbow', 'Crystal', 'Aurora', 'Cosmic', 'Divine']
  ...
  const getDistributionIcon = (mode: string) => {
    switch (mode) {
      case 'sell': return <DollarSign className="h-4 w-4 text-green-400" />
      case 'gift': return <Gift className="h-4 w-4 text-red-400" />
      case 'free': return <Heart className="h-4 w-4 text-pink-400" />
      case 'gameplay': return <Gamepad2 className="h-4 w-4 text-blue-400" />
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
            ...
            <div className="p-3 rounded-lg bg-green-900/30 border border-green-500/20">
              <div className="text-xl font-bold text-green-400">{nfts.filter(n => n.distributionMode === 'sell').length}</div>
            ...
            <div className="p-3 rounded-lg bg-red-900/30 border border-red-500/20">
              <div className="text-xl font-bold text-red-400">{nfts.filter(n => n.distributionMode === 'gift').length}</div>
            ...
            <div className="p-3 rounded-lg bg-pink-900/30 border border-pink-500/20">
              <div className="text-xl font-bold text-pink-400">{nfts.filter(n => n.distributionMode === 'free').length}</div>
              <div className="text-xs text-muted-foreground">Free</div>
            </div>
            <div className="p-3 rounded-lg bg-blue-900/30 border border-blue-500/20">
              <div className="text-xl font-bold text-blue-400">{nfts.filter(n => n.distributionMode === 'gameplay').length}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredNFTs.map((nft) => (
          <Card key={nft.id} className={`border-2 bg-gradient-to-br ${getRarityColor(nft.rarity)}/20 hover:scale-105 transition-all`}>
