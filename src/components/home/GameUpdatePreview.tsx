    ...
    <Card className="border-2 border-purple-500/50 bg-gradient-to-br from-purple-900/20 to-pink-900/20">
      ...
      <CardContent className="space-y-6">
        {upcomingGames.map((game, index) => (
          <div
            key={index}
            className="bg-black/30 border border-purple-500/20 rounded-lg p-4 space-y-3"
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {game.features.map((feature, idx) => (
                <div
                  key={idx}
                  className="bg-purple-900/20 border border-purple-500/30 rounded px-2 py-1 text-xs text-center"
                >
                  {feature}
                </div>
              ))}
