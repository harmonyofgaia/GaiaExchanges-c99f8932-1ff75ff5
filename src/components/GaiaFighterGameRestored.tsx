  ...
  const performAttack = (attackType: 'basic' | 'special' | 'ultimate') => {
    ...
    setTimeout(() => {
      setGameState(prev => {
        ...
        newState.gameLog = [...newState.gameLog.slice(-4), `💥 Eco Destroyer attacks for ${opponentDamage} damage!`]
        newState.playerTurn = true

        if (newState.player.health <= 0) {
          newState.gameActive = false
          newState.gameLog = [...newState.gameLog, '💀 Defeat! The environment needs your protection!']
          ...
          <div className="bg-red-900/20 border border-red-500/30 rounded-lg p-4">
            ...
            <div className="space-y-2">
              <div>
                <div className="flex justify-between text-sm">
                  <span>Health</span>
                  <span>{gameState.opponent.health}/{gameState.opponent.maxHealth}</span>
                </div>
                <Progress value={(gameState.opponent.health / gameState.opponent.maxHealth) * 100} className="h-2" />
              </div>
              <div className="grid grid-cols-2 gap-2 text-sm">
        ...
        <div className="bg-gradient-to-r from-green-900/20 via-blue-900/20 to-purple-900/20 border border-green-500/30 rounded-lg p-4">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Every victory in Gaia Fighter helps fund real environmental projects through GAiA token rewards
            </p>
          </div>
        </div>
      </CardContent>
