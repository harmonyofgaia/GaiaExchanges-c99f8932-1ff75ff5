...
interface LiveGAiAEvent {
  ...
  timestamp: Date;
  type:
    | "OFFICIAL_GAiA_TRANSACTION"
    | "OFFICIAL_GAiA_TRADE"
    | "OFFICIAL_HOLDER_ACTION"
    | "OFFICIAL_GAiA_UPDATE"
    | "OFFICIAL_SECURITY_EVENT";
  description: string;
  value: number;
  location: string;
...
export function LiveTrackingEngine() {
        ...
        <CardContent>
          <div className="max-h-64 overflow-y-auto space-y-2">
            {liveEvents.slice(0, 10).map((event) => (
              <div
                key={event.id}
                className="flex items-center justify-between p-2 rounded border border-border/50 bg-card/50"
              >
        ...
        <CardContent className="pt-6">
          <div className="text-center space-y-4">
            <h3 className="text-2xl font-bold text-yellow-400">
              🌍 OFFICIAL GAiA ECOSYSTEM WORLD DOMINATION
            </h3>
            <p className="text-lg text-muted-foreground max-w-4xl mx-auto">
              Our official GAiA system operates at{" "}
  ...
  function getEventBadgeColor(type: string) {
    switch (type) {
      case "OFFICIAL_GAiA_TRANSACTION":
        return "bg-green-600 text-white";
      case "OFFICIAL_GAiA_TRADE":
        return "bg-blue-600 text-white";
      case "OFFICIAL_HOLDER_ACTION":
        return "bg-purple-600 text-white";
      case "OFFICIAL_GAiA_UPDATE":
        return "bg-yellow-600 text-white";
      case "OFFICIAL_SECURITY_EVENT":
