...
export function LiveTVScreen() {
  ...
  const [upcomingShows, setUpcomingShows] = useState<LiveShow[]>([
    {
      ...
      description: 'Live painting session creating environmental awareness art while burning GAiA tokens for coral reef restoration',
    ...
    },
    {
      ...
      description: 'Building stunning virtual landscapes in real-time using our 3D tools, with token burns funding reforestation',
    ...
    },
    {
      ...
      description: 'Musical performance with underwater coral restoration footage, tokens burned for marine protection',
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setUpcomingShows(prev => prev.map(show => ({
        ...show,
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'Environmental Art': return 'bg-green-600'
      case 'Digital Art': return 'bg-purple-600'
      case 'Music & Environment': return 'bg-blue-600'
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            <div className="text-center p-3 bg-orange-900/30 border border-orange-500/20 rounded-lg">
              <div className="text-xl font-bold text-orange-400">{totalTokensBurned.toLocaleString()}</div>
            ...
            <div className="text-center p-3 bg-green-900/30 border border-green-500/20 rounded-lg">
              <div className="text-xl font-bold text-green-400">{totalEcologyFunding.toLocaleString()}</div>
            ...
            <div className="text-center p-3 bg-blue-900/30 border border-blue-500/20 rounded-lg">
              <div className="text-xl font-bold text-blue-400">{upcomingShows.filter(s => s.isLive).length}</div>
            ...
            <div className="text-center p-3 bg-purple-900/30 border border-purple-500/20 rounded-lg">
              <div className="text-xl font-bold text-purple-400">{upcomingShows.reduce((sum, show) => sum + show.viewers, 0).toLocaleString()}</div>
                  ...
                  <div className="bg-black/70 p-3 rounded-lg">
                    ...
                    <div className="w-full bg-gray-700 rounded-full h-2">
                      <div 
                        ...
                        style={{ width: `${Math.min(100, (currentShow.tokensCollected / 10000) * 100)}%` }}
                      />
                    </div>
                  </div>
                </div>
      ...
      <Card className="border-green-500/30 bg-gradient-to-r from-green-900/20 to-emerald-900/20">
        <CardHeader>
          <CardTitle className="text-green-400">💰 Pay for Show Access - Support Environment</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="text-center">
            <p className="text-sm text-muted-foreground mb-2">
              GAiA Contract: <code className="font-mono text-xs">{GAIA_TOKEN.CONTRACT_ADDRESS}</code>
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {upcomingShows.map((show) => (
              <div key={show.id} className="bg-muted/30 border border-border/50 rounded-lg p-4 space-y-3">
