        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="text-center">
              <Badge className={`${
                cloudEngineStatus === 'OPTIMAL' ? 'bg-green-600' : 
                cloudEngineStatus === 'INITIALIZING' ? 'bg-yellow-600' : 'bg-red-600'
                ...
                return (
                  <>
                    ...
                    <div>
                      ...
                      <p className="font-bold">{formatPercentage(token.price_change_percentage_24h)}</p>
                    </div>
                    <div>
          ...
          <div className="space-y-2">
            {tokens.slice(0, 10).map((token, index) => (
              <div key={token.id} className={`flex items-center justify-between p-3 rounded-lg ${
                token.id === selectedToken ? 'bg-blue-900/30 border border-blue-500/30' : 'bg-muted/30'
              }`}>
                <div className="flex items-center gap-3">
                  <span className="text-muted-foreground">#{token.market_cap_rank || index + 1}</span>
                  <div>
