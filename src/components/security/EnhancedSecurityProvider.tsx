...
interface SecurityContextType {
  logSecurityEvent: (action: string, details?: Record<string, any>, riskScore?: number) => Promise<void>;
  createSecurityIncident: (type: string, severity: "low" | "medium" | "high" | "critical", details?: Record<string, any>) => Promise<void>;
  checkRateLimit: (identifier: string, actionType: string, maxAttempts?: number, windowMinutes?: number) => Promise<boolean>;
}
...
export function EnhancedSecurityProvider({ children }: EnhancedSecurityProviderProps) {
  const { logSecurityEvent, createSecurityIncident, checkRateLimit } = useSecurityMonitoring();
  const { user } = useAuth();
    ...
    if (user) {
      ...
      const handleActivity = () => {
        ...
        if (now - lastActivity > 30 * 60 * 1000) {
          logSecurityEvent("user_activity_resumed", { 
      ...
      return () => {
        ...
        // Log session end
        logSecurityEvent("user_session_end", { 
