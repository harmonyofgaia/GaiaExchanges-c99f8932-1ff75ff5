...
export function PrehistoricGuardian() {
    ...
    const improveGuardian = () => {
      ...
      setGuardianStats((prev) => {
        ...
        if (newStats.experience >= newStats.level * 100) {
          ...
          toast.success(
            `🦕 Prehistoric Guardian leveled up to ${newStats.level}!`,
            {
        ...
        if (detectedProblems.length > 0) {
          try {
            await supabase.from("security_events").insert({
              ...
              severity: detectedProblems.some((p) => p.severity === "high")
                ? 70
                : 30,
        ...
        case "security":
          ...
          try {
            await supabase.from("security_events").insert({
              ...
              event_category: "SYSTEM",
              event_details: {
                description: `Guardian resolved: ${problem.description}`,
              },
              severity: 70,
              ip_address: "127.0.0.1",
            });
  ...
  const toggleGuardian = () => {
    ...
    toast.success(
      `🦕 Guardian ${!guardianActive ? "activated" : "deactivated"}!`,
      {
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-3 bg-red-900/20 rounded-lg border border-red-500/30">
              <div
                className={`text-xl font-bold ${getStatColor(guardianStats.strength)}`}
              >
            ...
            <div className="text-center p-3 bg-blue-900/20 rounded-lg border border-blue-500/30">
              <div
                className={`text-xl font-bold ${getStatColor(guardianStats.intelligence)}`}
              >
                {guardianStats.intelligence.toFixed(1)}%
              </div>
              <div className="text-xs text-muted-foreground">Intelligence</div>
              <Progress
                value={guardianStats.intelligence}
                className="h-1 mt-1"
              />
            </div>
            ...
            <div className="text-center p-3 bg-yellow-900/20 rounded-lg border border-yellow-500/30">
              <div
                className={`text-xl font-bold ${getStatColor(guardianStats.speed)}`}
              >
            ...
            <div className="text-center p-3 bg-purple-900/20 rounded-lg border border-purple-500/30">
              ...
              <Progress
                value={
                  (guardianStats.experience / (guardianStats.level * 100)) * 100
                }
                className="h-1 mt-1"
              />
            </div>
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div className="p-2 bg-red-900/20 rounded border border-red-500/30">
              <div className="text-lg font-bold text-red-400">
                {guardianStats.threatsNeutralized}
              </div>
              <div className="text-xs text-muted-foreground">
                Threats Stopped
              </div>
            ...
            <div className="p-2 bg-blue-900/20 rounded border border-blue-500/30">
              <div className="text-lg font-bold text-blue-400">
                {guardianStats.systemsProtected}
              </div>
              <div className="text-xs text-muted-foreground">
                Systems Protected
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={
                    guardianActive ? "text-green-400" : "text-gray-400"
                  }
                >
                  {guardianActive
                    ? "Guardian Active - Continuous Protection"
                    : "Guardian Dormant"}
                </span>
              </div>
              <Badge
                className={guardianActive ? "bg-green-600" : "bg-gray-600"}
              >
          ...
          <div className="space-y-3 max-h-60 overflow-y-auto">
            {problems.length === 0 ? (
              <div className="text-center text-muted-foreground py-8">
                ...
                <p className="text-sm">
                  Guardian found no issues - Everything running smoothly
                </p>
              </div>
            ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {problem.location} •{" "}
                        {problem.timestamp.toLocaleTimeString()}
                      </p>
                    </div>
                    {problem.resolved ? (
