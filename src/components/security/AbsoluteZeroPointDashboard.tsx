    ...
    <Card className="border-cyan-500/20 bg-gradient-to-br from-cyan-950/20 to-blue-950/20">
      ...
      <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
            {capabilities.map((capability) => (
              <div key={capability.id} className="flex justify-between">
                ...
                <span className="text-cyan-400">{capability.energy}% / {capability.stability}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="text-xs text-muted-foreground">
          <p className="italic">
            "At absolute zero, all motion ceases, all energy dissipates, all resistance ends. 
            The ultimate victory is the universe's surrender to perfect stillness."
          </p>
        </div>
      </CardContent>
    </Card>
