...
export function SelfTrainingAnimal() {
  const [systemFiles, setSystemFiles] = useState<SystemFile[]>([
    { path: 'src/App.tsx', status: 'perfect', improvements: 0, lastCheck: new Date() },
    { path: 'src/pages/*.tsx', status: 'optimizing', improvements: 12, lastCheck: new Date() },
    { path: 'src/components/**/*.tsx', status: 'learning', improvements: 34, lastCheck: new Date() },
    { path: 'src/security/**/*.tsx', status: 'evolving', improvements: 89, lastCheck: new Date() }
  ])
  ...
  useEffect(() => {
    const selfTrainingEngine = () => {
      ...
      setSystemFiles(prev => prev.map(file => ({
        ...file,
          ...
          {systemFiles.map((file, index) => (
            <div key={index} className="flex items-center justify-between p-2 bg-black/30 rounded">
              ...
              <div className="flex items-center gap-2">
                <Badge className={`${
                  file.status === 'perfect' ? 'bg-green-600' :
                  file.status === 'evolving' ? 'bg-purple-600' :
                  file.status === 'learning' ? 'bg-blue-600' :
                  'bg-orange-600'
                } text-xs`}>
                  {file.status}
                </Badge>
