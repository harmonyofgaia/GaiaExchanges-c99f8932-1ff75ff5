...
export function NetworkDefenseGrid() {
  const [networkNodes, setNetworkNodes] = useState<NetworkNode[]>([
    { id: 'node-us-east', name: 'US East Defense Hub', location: 'New York, USA', status: 'active', threatsBlocked: 234, bandwidthUsed: 87, uptime: 99.9 },
    { id: 'node-us-west', name: 'US West Defense Hub', location: 'San Francisco, USA', status: 'blocking', threatsBlocked: 156, bandwidthUsed: 92, uptime: 99.7 },
    { id: 'node-eu-central', name: 'EU Central Defense Hub', location: 'Frankfurt, Germany', status: 'defending', threatsBlocked: 189, bandwidthUsed: 78, uptime: 99.8 },
    { id: 'node-asia-east', name: 'Asia East Defense Hub', location: 'Tokyo, Japan', status: 'attacking', threatsBlocked: 267, bandwidthUsed: 95, uptime: 99.6 },
    { id: 'node-oceania', name: 'Oceania Defense Hub', location: 'Sydney, Australia', status: 'active', threatsBlocked: 98, bandwidthUsed: 65, uptime: 99.9 },
    { id: 'node-africa', name: 'Africa Defense Hub', location: 'Cape Town, South Africa', status: 'blocking', threatsBlocked: 123, bandwidthUsed: 73, uptime: 99.5 },
    { id: 'node-south-america', name: 'South America Defense Hub', location: 'São Paulo, Brazil', status: 'defending', threatsBlocked: 145, bandwidthUsed: 81, uptime: 99.4 },
    { id: 'node-middle-east', name: 'Middle East Defense Hub', location: 'Dubai, UAE', status: 'active', threatsBlocked: 167, bandwidthUsed: 89, uptime: 99.8 }
  ])
    ...
    const updateNetworkDefenseGrid = () => {
      ...
      try {
        ...
        setNetworkNodes(prev => prev.map(node => {
          const statuses: ('active' | 'blocking' | 'attacking' | 'defending')[] = ['active', 'blocking', 'attacking', 'defending']
          
          // Randomly update node status and metrics
          if (Math.random() < 0.3) {
            const newStatus = statuses[Math.floor(Math.random() * statuses.length)]
            const additionalThreats = newStatus === 'blocking' || newStatus === 'attacking' ? Math.floor(Math.random() * 5) + 1 : 0
            
            return {
              ...node,
              status: newStatus,
              threatsBlocked: node.threatsBlocked + additionalThreats,
              bandwidthUsed: Math.min(100, Math.max(50, node.bandwidthUsed + (Math.random() - 0.5) * 10)),
  ...
  const getNodeStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'blocking': return 'bg-yellow-600'
  ...
  const getNodeStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'blocking': return <Ban className="h-3 w-3" />
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {networkNodes.map((node) => (
          <Card key={node.id} className="border-indigo-500/30 bg-gradient-to-br from-indigo-900/20 to-purple-900/20">
