      ...
      <CardContent>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="text-center p-4 bg-blue-900/30 rounded-lg">
            <div className={`text-2xl font-bold flex items-center justify-center gap-1 ${
              ...
              {tokenData.priceChange24h >= 0 ? <TrendingUp className="h-5 w-5" /> : <TrendingDown className="h-5 w-5" />}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center p-3 bg-cyan-900/20 rounded-lg">
            <div className="text-lg font-bold text-cyan-400">{tokenData.holders.toLocaleString()}</div>
            <div className="text-xs text-muted-foreground">Token Holders</div>
          </div>
          
          <div className="text-center p-3 bg-yellow-900/20 rounded-lg">
            <div className="text-lg font-bold text-yellow-400">{tokenData.transactions24h.toLocaleString()}</div>
        ...
        <div className="mt-4 text-center text-xs text-muted-foreground">
          Last updated: {tokenData.lastUpdated.toLocaleTimeString()} | Contract: {GAIA_TOKEN.CONTRACT_ADDRESS.slice(0, 20)}...
        </div>
      </CardContent>
    </Card>
  )
