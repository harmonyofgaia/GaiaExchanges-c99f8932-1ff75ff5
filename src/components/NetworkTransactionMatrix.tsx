...
export function NetworkTransactionMatrix() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const transactionTypes: NetworkTransaction["type"][] = [
        "buy",
        "sell",
        "transfer",
        "burn",
      ...
      const newTransaction: NetworkTransaction = {
        ...
        price: 3.0 + (Math.random() - 0.5) * 0.5,
        fromWallet:
          randomType === "buy"
            ? "Exchange Wallet"
            : `Holder Wallet #${Math.floor(Math.random() * 9999)}`,
        toWallet:
          randomType === "sell"
            ? "Exchange Pool"
            : `Community Wallet #${Math.floor(Math.random() * 9999)}`,
        timestamp: new Date(),
              ...
              {networkTransactions.map((tx, index) => (
                <div
                  ...
                  className={`flex items-center justify-between p-3 rounded-lg border transition-all duration-500 ${
                    index === 0
                      ? "border-yellow-500/50 bg-yellow-900/10 animate-pulse"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-300">
                        From:{" "}
                        {tx.fromWallet.length > 25
                          ? `${tx.fromWallet.slice(0, 25)}...`
                          : tx.fromWallet}
                      </div>
                    </div>
                  </div>
