...
import { ENHANCED_GAIA_PROJECTS, PROJECT_WALLETS, PROJECT_INVESTMENT_TIERS } from '@/constants/enhanced-gaia-projects'
...
export function EnhancedGreenInvestments() {
  ...
  const getProjectIcon = (id: string) => {
    switch (id) {
      case 'land-recovery-7phase': return <Leaf className="h-6 w-6" />
      case 'pvas-water-cleanup': return <Droplets className="h-6 w-6" />
      case 'community-relocation-eldercare': return <Home className="h-6 w-6" />
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {project.isFeatured && project.researchPhases && (
                      <div className="bg-emerald-900/20 border border-emerald-500/20 rounded-lg p-3">
                        <h4 className="text-sm font-medium text-emerald-400 mb-2">Research Phases</h4>
                        <div className="space-y-1">
                          {project.researchPhases.slice(0, 3).map((phase, index) => (
                            <div key={index} className="text-xs text-emerald-300/80 flex items-center gap-1">
                    ...
                    {wallet && (
                      <div className="bg-blue-900/20 border border-blue-500/20 rounded-lg p-3">
                        ...
                        <p className="text-xs text-blue-300/80 font-mono break-all">{wallet.address}</p>
                        <p className="text-xs text-blue-300/60 mt-1">Target: ${wallet.fundingTarget.toLocaleString()}</p>
                      </div>
                    )}
            ...
            <CardContent>
              <div className="grid gap-4">
                {Object.entries(PROJECT_INVESTMENT_TIERS).map(([tier, config]) => (
                  <div key={tier} className="bg-purple-800/20 border border-purple-400/20 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-bold text-purple-300 capitalize">{tier} Tier</h3>
                        <p className="text-sm text-purple-400">
                          {config.min.toLocaleString()} - {config.max === Infinity ? '∞' : config.max.toLocaleString()} GAIA
                        </p>
                      </div>
                      <Badge className={`bg-${tier === 'diamond' ? 'yellow' : tier === 'platinum' ? 'gray' : tier === 'gold' ? 'yellow' : tier === 'silver' ? 'gray' : 'orange'}-600`}>
                        {config.reward}
                      </Badge>
                    </div>
                  </div>
        ...
        <TabsContent value="wallets" className="space-y-6">
          <div className="grid gap-4">
            {Object.entries(PROJECT_WALLETS).map(([projectId, wallet]) => (
              <Card key={projectId} className="bg-gradient-to-br from-blue-900/30 to-indigo-900/30 border-blue-500/30">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-blue-300/60">
                        Target: ${wallet.fundingTarget.toLocaleString()} | Currency: {wallet.currency}
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              {investmentAmount >= 100 && (
                <div className="bg-green-800/20 border border-green-500/20 rounded p-3">
                  <p className="text-sm text-green-400">
                    Tier: <span className="font-bold">{getInvestmentTier(investmentAmount)?.toUpperCase()}</span>
                  </p>
                  <p className="text-xs text-green-300/80">
                    {PROJECT_INVESTMENT_TIERS[getInvestmentTier(investmentAmount) as keyof typeof PROJECT_INVESTMENT_TIERS]?.reward}
                  </p>
                </div>
              )}
