  ...
  useEffect(() => {
    const cleanup = setInterval(() => {
      setExplosions(prev => prev.filter(explosion => 
        Date.now() - explosion.timestamp < 1000
      ))
    }, 100)
  ...
  const fireWeapon = async () => {
    ...
    setWorms(prev => prev.map(worm => {
      ...
      if (distance <= weapon.radius && worm.health > 0) {
        ...
        const damage = Math.floor(weapon.damage * damageFactor) + Math.floor(Math.random() * 20) - 10
        ...
        <div className="flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-cyan-400 font-bold">Wind: {windStrength > 0 ? '+' : ''}{windStrength}</span>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              {getWormsByTeam('player').map(worm => (
                <div key={worm.id} className="mb-2 p-2 bg-green-900/20 rounded border border-green-500/20">
            ...
            <div>
              ...
              {getWormsByTeam('enemy').map(worm => (
                <div key={worm.id} className="mb-2 p-2 bg-red-900/20 rounded border border-red-500/20">
                  <div className="flex justify-between items-center mb-1">
