...
export function SnakeWormsIntegration() {
  ...
  const moveSnake = useCallback(() => {
    ...
    setSnake(currentSnake => {
      ...
      if (newSnake.length > 15) { // Max snake length
        newSnake.pop()
      }
    ...
    setWorms(prev => prev.map(w => {
      if (w.id === worm.id) {
        ...
        if (newHealth === 0) {
          ...
          setTimeout(() => {
            setExplosions(prev => prev.filter(pos => !w.segments.some(seg => seg.x === pos.x && seg.y === pos.y)))
          }, 1000)
  ...
  const renderBoard = () => {
    return Array.from({ length: BOARD_SIZE * BOARD_SIZE }).map((_, index) => {
      ...
      const isWorm = worms.some(worm => worm.segments.some(segment => segment.x === x && segment.y === y))
    ...
    <Card className="border-orange-500/30 bg-gradient-to-br from-orange-900/20 to-red-900/20">
      <CardHeader>
        ...
        <div className="flex gap-4 flex-wrap">
          ...
          <Badge className="bg-blue-600 text-white">GAiA: {gameStats.gaiaTokensEarned.toFixed(1)}</Badge>
      ...
      <CardContent className="space-y-4">
        ...
        {worms.length === 0 && gameState === 'playing' && (
          <div className="text-center p-4 bg-green-900/20 border border-green-500/30 rounded">
            ...
            <p className="text-green-400">GAiA Tokens Earned: {gameStats.gaiaTokensEarned.toFixed(1)}</p>
