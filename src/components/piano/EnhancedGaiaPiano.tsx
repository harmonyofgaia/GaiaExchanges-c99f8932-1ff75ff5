...
function PianoKey({ 
  ...
  return (
    <div 
      ...
      onClick={onClick}
    >
      <div className={`h-full ${isBlackKey ? 
        ...
        <div className={`${isBlackKey ? 'bg-gray-800' : 'bg-white/80'} rounded-full p-3 mb-3 shadow-lg`}>
          <Icon className={`${isBlackKey ? 'h-8 w-8' : 'h-10 w-10'} text-${color}-${isBlackKey ? '400' : '600'} group-hover:animate-bounce`} />
        </div>
        <span className={`text-sm font-bold ${isBlackKey ? `text-${color}-200 text-center transform -rotate-90 whitespace-nowrap` : `text-gray-800 text-center leading-tight`}`}>
          {isBlackKey ? title : title.split(' ').join('\n')}
        </span>
        {!isBlackKey && (
          <div className={`w-full h-1 bg-${color}-200 rounded-full mt-2 opacity-0 group-hover:opacity-100 transition-opacity`}>
            <div className={`h-full bg-${color}-600 rounded-full w-5/6 animate-pulse`}></div>
          </div>
        )}
      </div>
      ...
      <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-6 opacity-0 group-hover:opacity-100 transition-all duration-700 scale-50 group-hover:scale-100 z-50 pointer-events-none">
        <div className={`bg-black/98 backdrop-blur-2xl rounded-3xl border-2 border-${color}-500/70 p-8 w-96 shadow-[0_0_40px_${glowColor}]`}>
          <div className="text-center mb-6">
            <div className={`bg-gradient-to-r from-${color}-600 to-${color}-400 rounded-full p-4 mx-auto w-fit mb-4`}>
              <Icon className="h-16 w-16 text-white" />
            </div>
            <h4 className={`text-2xl font-bold text-${color}-400 mb-2`}>{title}</h4>
            <p className="text-gray-300">{description}</p>
          </div>
          <div className={`h-48 overflow-hidden rounded-xl border-2 border-${color}-500/40 bg-gradient-to-br from-${color}-900/30 to-${color}-800/20`}>
            <Suspense fallback={<div className={`animate-pulse bg-${color}-500/20 h-full flex items-center justify-center`}><span className={`text-${color}-300`}>Loading...</span></div>}>
              <Component />
            </Suspense>
          </div>
          <div className="mt-4 text-center">
            <Badge className={`bg-${color}-600 text-white px-4 py-2`}>Click to explore!</Badge>
          </div>
        </div>
      </div>
    </div>
...
export function EnhancedGaiaPiano({ onCategorySelect }: EnhancedGaiaPianoProps) {
  const whiteKeys = [
    {
      icon: Trophy,
    ...
    <div className="text-center space-y-8">
      <div className="space-y-6">
        ...
        <p className="text-2xl text-muted-foreground max-w-3xl mx-auto font-medium">
          ✨ Touch the keys of change - Experience the symphony of sustainability ✨
        </p>
        <div className="flex justify-center items-center gap-4 text-lg">
          <span className="animate-bounce">🎵</span>
          <span className="text-amber-400 font-semibold">Hover to preview • Click to explore</span>
        ...
        <div className="bg-gradient-to-b from-amber-900/50 via-amber-800/70 to-amber-700/90 rounded-3xl p-12 border-8 border-amber-600/80 shadow-[0_0_60px_rgba(245,158,11,0.6)] backdrop-blur-sm">
          ...
          <div className="mb-8 text-center">
            <div className="inline-block bg-gradient-to-r from-amber-100 to-amber-50 rounded-xl p-6 shadow-2xl border-4 border-amber-400 transform -rotate-1 hover:rotate-0 transition-transform duration-500">
              <h3 className="text-3xl font-bold text-amber-900 mb-3">🎼 Environmental Symphony</h3>
              <p className="text-amber-800 text-lg">Each key unlocks sustainable actions for our planet</p>
            </div>
          </div>
          ...
          <div className="relative bg-gradient-to-br from-amber-900 via-amber-800 to-amber-700 rounded-2xl p-10 shadow-[inset_0_0_30px_rgba(0,0,0,0.7)] border-6 border-amber-600/90">
            ...
            <div className="text-center mt-8">
              <div className="inline-block bg-gradient-to-r from-amber-800 via-amber-700 to-amber-600 text-amber-100 px-8 py-4 rounded-full font-bold text-xl shadow-[0_0_30px_rgba(245,158,11,0.5)] border-2 border-amber-500/50">
                ...
                <div className="text-sm font-normal mt-1 opacity-80">Environmental Symphony • Play to Explore</div>
              </div>
            </div>

            {/* Decorative Musical Elements */}
            <div className="absolute top-4 left-4 text-2xl animate-bounce delay-300">🎼</div>
            <div className="absolute top-8 right-8 text-2xl animate-bounce delay-700">🎶</div>
            <div className="absolute bottom-16 left-8 text-xl animate-bounce delay-500">♪</div>
            <div className="absolute bottom-20 right-12 text-xl animate-bounce delay-900">♫</div>
          </div>
        </div>
      </div>
    </div>
