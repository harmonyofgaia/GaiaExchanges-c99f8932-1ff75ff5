          ...
          <Card key={landscape.id} className="border-blue-500/20 bg-blue-900/10">
            <CardHeader>
              <div className="flex items-start justify-between">
                ...
                <Badge className={landscape.status === 'Published' ? 'bg-green-600' : 'bg-yellow-600'}>
                  {landscape.status}
                </Badge>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="ml-2 text-green-400 font-semibold">{landscape.downloads.toLocaleString()}</span>
                </div>
              </div>
      ...
      <Card className="border-green-500/20 bg-green-900/10">
        <CardContent className="pt-6 text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              <div className="text-2xl font-bold text-blue-400">{landscapes.filter(l => l.status === 'Published').length}</div>
              <div className="text-sm text-muted-foreground">Published</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-purple-400">{landscapes.reduce((sum, l) => sum + l.downloads, 0).toLocaleString()}</div>
              <div className="text-sm text-muted-foreground">Total Downloads</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-yellow-400">{(landscapes.reduce((sum, l) => sum + l.rating, 0) / landscapes.length).toFixed(1)}</div>
