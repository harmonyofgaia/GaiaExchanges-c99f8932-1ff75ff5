...
export function GlobalRelocationMap() {
  ...
  const uploadRelocationContent = (type: "photo" | "video" | "stream") => {
    ...
    toast.success(
      `📹 ${contentType.charAt(0).toUpperCase() + contentType.slice(1)} upload activated!`,
      {
    ...
    <div className="space-y-6">
      ...
      <Card className="min-h-[500px]">
        <CardContent className="p-0">
          <div
            ref={mapRef}
            className="relative w-full h-[500px] bg-gradient-to-b from-blue-900 to-green-900 rounded overflow-hidden"
          >
            {mapView === "earth" && (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                    {animals
                      .filter((a) => a.relocationReady)
                      .map((animal) => (
                        <div
                          key={animal.id}
                          className="bg-black/40 p-4 rounded border border-green-500/20"
                        >
