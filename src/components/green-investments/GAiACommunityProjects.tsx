        ...
        <CardContent className="space-y-6">
          <div className="flex flex-wrap gap-2">
            {project.tags.map((tag: string, index: number) => (
              <Badge key={index} variant="outline" className="border-purple-500/50 text-purple-300 bg-purple-900/20">
                {tag}
              </Badge>
            ))}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {project.researchPhases?.map((phase: string, index: number) => (
                <div key={index} className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${index < 2 ? 'bg-green-400' : 'bg-gray-500'}`} />
