...
export function ArtistStreamingPlatform() {
  ...
  useEffect(() => {
    ...
    const stats = liveStreams.reduce((acc, stream) => ({
    ...
    }), { totalViewers: 0, totalEarnings: 0, totalTokensBurned: 0, activeStreams: 0 })
    ...
    try {
      ...
      if (burnSuccess) {
        ...
        setLiveStreams(prev => prev.map(s => 
          s.id === stream.id 
            ? { ...s, earnings: s.earnings + tipAmount, tokensBurned: s.tokensBurned + burnAmount }
            : s
        ))
      }
    } catch (error) {
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'Digital Art': return 'bg-purple-600'
      case 'Workshop': return 'bg-green-600'
      case 'NFT Art': return 'bg-blue-600'
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'Digital Art': return <Camera className="h-4 w-4" />
      case 'Workshop': return <Star className="h-4 w-4" />
      case 'NFT Art': return <Crown className="h-4 w-4" />
      ...
      <Card className="border-2 border-purple-500/50 bg-gradient-to-br from-purple-900/30 to-pink-900/30">
        <CardHeader>
          ...
          <p className="text-muted-foreground">
            Support artists with GAiA tokens while contributing to environmental impact through token burning
          </p>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-blue-900/30 border border-blue-500/20 rounded-lg">
              <div className="text-2xl font-bold text-blue-400">{streamingStats.totalViewers.toLocaleString()}</div>
            ...
            <div className="text-center p-4 bg-green-900/30 border border-green-500/20 rounded-lg">
              <div className="text-2xl font-bold text-green-400">{streamingStats.activeStreams}</div>
            ...
            <div className="text-center p-4 bg-yellow-900/30 border border-yellow-500/20 rounded-lg">
              <div className="text-2xl font-bold text-yellow-400">{streamingStats.totalEarnings.toLocaleString(undefined, {maximumFractionDigits: 0})}</div>
            ...
            <div className="text-center p-4 bg-orange-900/30 border border-orange-500/20 rounded-lg">
              <div className="text-2xl font-bold text-orange-400">{streamingStats.totalTokensBurned}</div>
        ...
        {liveStreams.map((stream) => (
          <Card 
            ...
            className={`border-2 hover:scale-105 transition-all duration-300 ${
              selectedStream === stream.id ? 'border-purple-500 ring-4 ring-purple-500/20' : 'border-border/50'
            }`}
          >
            <CardContent className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <div className="text-lg font-bold text-pink-400">{stream.likes.toLocaleString()}</div>
              ...
              <div className="space-y-2">
                ...
                <Progress value={(stream.tokensBurned / stream.tokensEarned) * 100} className="h-2" />
              </div>
        ...
        <CardContent>
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Every tip and interaction burns GAiA tokens for real environmental impact while supporting amazing artists.
              Join the creative community that makes a difference!
            </p>
