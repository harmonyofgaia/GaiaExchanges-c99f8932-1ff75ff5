...
export function PersistentEvolutionEngine() {
  ...
  useEffect(() => {
    ...
    const calculateOfflineGrowth = () => {
      ...
      if (offlineHours > 0.1) { // More than 6 minutes offline
        const offlineGrowth = Math.pow(metrics.evolutionRate, offlineHours * 60) * metrics.offlineGrowthMultiplier
  ...
  useEffect(() => {
    ...
    const currentMilestone = powerMilestones.find(m => 
  ...
  useEffect(() => {
    if (DEBUG_MODE && Math.floor(metrics.totalSystemPower) % 100 < 2) {
      ...
      console.log(`🌍 GPS Tracking: ${Math.floor(metrics.powerLevels.gpsTracking).toLocaleString()}`)
      console.log(`🦎 Animal Rescue: ${Math.floor(metrics.powerLevels.animalRescue).toLocaleString()}`)
      console.log(`⛓️ Blockchain: ${Math.floor(metrics.powerLevels.blockchain).toLocaleString()}`)
      console.log(`🏔️ Landscape Builder: ${Math.floor(metrics.powerLevels.landscapeBuilder).toLocaleString()}`)
      console.log(`👑 Admin Control: ${Math.floor(metrics.powerLevels.adminControl).toLocaleString()}`)
    }
  }, [metrics])
